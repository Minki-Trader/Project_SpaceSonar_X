...
def _is_full_project(repo_root: Path) -> bool:
    return all(
        ...
        for rel_path in [
            ...
            "foundation/validation/control_plane_validator.py",
            "foundation/validation/active_record_validator.py",
        ]
    )
...
def _validation_errors(repo_root: Path) -> list[str]:
    ...
    if _is_full_project(repo_root):
        from foundation.validation.active_record_validator import validate as active_validate
        from foundation.validation.control_plane_validator import validate as control_validate

        errors.extend(f"control_plane_validator: {item}" for item in control_validate(repo_root))
        errors.extend(f"active_record_validator: {item}" for item in active_validate(repo_root))
    return errors

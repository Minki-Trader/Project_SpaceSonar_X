...
PARENT_WORK_ITEM_ID = "work_wave02_execution_liquidity_l4_runtime_execution_v0"
WORK_ITEM_ID = "work_wave02_execution_liquidity_l4_pair_judgment_v0"
NEXT_WORK_ITEM_ID = "work_wave02_execution_liquidity_l5_routing_decision_v0"
...
PAIR_CLOSEOUT = Path(
    ...
    "work_wave02_execution_liquidity_l4_pair_judgment_v0_closeout.yaml"
)
...
RUN_REFS = (
    Path("lab/campaigns/campaign_us100_wave02_execution_liquidity_surface_v0")
    / "sweeps"
    / SWEEP_ID
    / "run_refs.csv"
)
...
CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l4_pair_judgment_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l5_routing_decision_pending_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
STATUS = "wave02_execution_liquidity_l4_pair_judgment_completed_l5_routing_decision_pending"
NEXT_ACTION = (
    "decide whether Wave02 execution/liquidity pairs justify a bounded decision-execution adapter "
...
def aggregate_pairs(started_at_utc: str, command_argv: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    summary = {
        "version": "wave02_execution_liquidity_l4_pair_judgment_summary_v1",
        "summary_id": "wave02_execution_liquidity_l4_pair_judgment_summary_v0",
        ...
        "writer_scope_self_check": {"status": "pending_after_write"},
        ...
        "judgment": {
            "result_subject": "Wave02 execution/liquidity paired validation/research_oos L4 score runtime observations",
            ...
            "tested_factor": "execution/liquidity score-probe runtime follow-through for six preserved proxy clues",
            "kpi_interpretation": (
            ...
            "directional_effect_hypothesis": (
                "execution/liquidity semantics remain runtime-testable enough to justify a decision-execution adapter review, "
        ...
        "prevention_memory": [
            "Wave02 execution/liquidity score-probe pair judgment must precede any L5 routing decision.",
            ...
            "A preserved ELQ score clue needs decision-execution adapter evidence before any candidate-specific L5 manifest.",
        ],
        "unresolved_blockers": ["Wave02_execution_liquidity_L5_routing_decision_pending"],
        "reopen_conditions": [
            "rerun pair aggregation if a Wave02 ELQ L4 runtime attempt manifest, score telemetry summary, or tester report receipt changes",
...
def build_closeout(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "unresolved_blockers_or_none": summary["unresolved_blockers"],
...
def upsert_artifact_registry(summary: dict[str, Any]) -> None:
    ...
    put(
        "artifact_wave02_execution_liquidity_l4_pair_judgment_summary_v0",
        "l4_pair_judgment_summary",
        PAIR_SUMMARY,
        "source-of-truth summary for Wave02 ELQ paired L4 runtime score judgment",
    )
    put(
        "artifact_wave02_execution_liquidity_l4_pair_judgment_index_v0",
        "l4_pair_judgment_index",
        PAIR_INDEX,
        "compact index of Wave02 ELQ paired validation/research_oos L4 judgments",
    )
    put(
        "artifact_wave02_execution_liquidity_l4_pair_judgment_closeout_v0",
        "work_closeout",
        PAIR_CLOSEOUT,
        "closeout for Wave02 ELQ L4 pair judgment work",
    )
...
def update_control_records(summary: dict[str, Any]) -> None:
    ...
    next_work = {
        ...
        "acceptance_criteria": [
            "decide whether Wave02 execution/liquidity score-observation pairs justify bounded decision-execution adapter work",
        ...
        "summary": "Route Wave02 ELQ pair judgment outcomes without promoting score probes to candidates.",
        "provenance": {
            "source": WORK_ITEM_ID,
            "campaign_id": CAMPAIGN_ID,
            "source_of_truth": PAIR_SUMMARY.as_posix(),
        ...
        "unresolved_blockers": summary["unresolved_blockers"],
        "reopen_conditions": summary["reopen_conditions"],
        "status": "wave02_execution_liquidity_l5_routing_decision_pending",
    ...
    goal["next_work_item"] = {
        "work_item_id": NEXT_WORK_ITEM_ID,
        "path": NEXT_WORK_ITEM.as_posix(),
        "summary": "Wave02 ELQ L5 routing decision pending; no candidate claim.",
    }
    ...
    counts["wave02_execution_liquidity_l4_pair_judgment"] = summary["counts"]
    elq = workspace.setdefault("wave02_execution_liquidity_l4_materialization", {})
    elq["status"] = STATUS
    elq["claim_boundary"] = NEXT_CLAIM_BOUNDARY
    elq["l4_pair_judgment_summary"] = PAIR_SUMMARY.as_posix()
    elq["l4_pair_judgment_index"] = PAIR_INDEX.as_posix()
    elq["l4_pair_judgment_status"] = summary["status"]
    elq["l4_pair_judgment_counts"] = summary["counts"]
...
def smoke_pair_outputs(summary: dict[str, Any], pair_rows: list[dict[str, Any]]) -> list[str]:
    ...
    if loaded_summary.get("writer_contract_version") != WRITER_CONTRACT_VERSION:
        errors.append("pair summary writer_contract_version mismatch")

    registry_rows = read_csv_rows(REPO_ROOT / ARTIFACT_REGISTRY)
    by_id = {row.get("artifact_id"): row for row in registry_rows}
    for artifact_id, path in [
        ("artifact_wave02_execution_liquidity_l4_pair_judgment_summary_v0", PAIR_SUMMARY),
        ("artifact_wave02_execution_liquidity_l4_pair_judgment_index_v0", PAIR_INDEX),
        ("artifact_wave02_execution_liquidity_l4_pair_judgment_closeout_v0", PAIR_CLOSEOUT),
    ]:
    ...
    if workspace.get("unresolved_blockers") != ["Wave02_execution_liquidity_L5_routing_decision_pending"]:
    ...
    if next_work.get("status") != "wave02_execution_liquidity_l5_routing_decision_pending":
...
def writer_scope_self_check(errors: list[str], pair_rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate Wave02 execution/liquidity L4 validation/research_oos pair judgments.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    command_argv = [
        ...
        "foundation/pipelines/aggregate_wave02_execution_liquidity_l4_pair_judgments.py",
        *(argv or []),
    ]
    if args.smoke_only:
        ...
        errors = smoke_pair_outputs(summary, pair_rows)
        if not errors:
            summary["writer_scope_self_check"] = writer_scope_self_check(errors, pair_rows)
            write_yaml(PAIR_SUMMARY, summary)
            write_yaml(PAIR_CLOSEOUT, build_closeout(summary))
            upsert_artifact_registry(summary)
            if args.write_control_records:
    ...
    if errors:
        ...
        return 1
    print(
        "wave02 ELQ l4 pair judgment writer-smoke passed: "

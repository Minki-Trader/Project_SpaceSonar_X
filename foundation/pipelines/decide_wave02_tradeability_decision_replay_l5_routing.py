...
import foundation.pipelines.aggregate_wave02_tradeability_decision_replay_pair_judgments as pair_writer
...
WORK_ITEM_ID = "work_wave02_tradeability_decision_replay_l5_routing_decision_v0"
NEXT_WORK_ITEM_ID = "work_wave02_tradeability_l5_candidate_runtime_evidence_preparation_v0"
...
ROUTING_CLOSEOUT = Path(
    ...
    "work_wave02_tradeability_decision_replay_l5_routing_decision_v0_closeout.yaml"
)
...
CLAIM_BOUNDARY = (
    "wave02_decision_replay_l5_routing_decision_only_"
...
STATUS = "wave02_l5_candidate_manifest_opened_runtime_evidence_preparation_pending"
NEXT_ACTION = "prepare candidate-specific L5 runtime evidence for opened Wave02 decision replay targets"
...
def build_summary(decision_rows: list[dict[str, Any]], started_at_utc: str, command_argv: list[str]) -> dict[str, Any]:
    ...
    return {
        "version": "wave02_tradeability_decision_replay_l5_routing_decision_summary_v1",
        "summary_id": "wave02_tradeability_decision_replay_l5_routing_decision_summary_v0",
        ...
        "unresolved_blockers": ["Wave02_candidate_specific_L5_runtime_evidence_preparation_pending"],
        "reopen_conditions": [
...
def upsert_artifact_registry(summary: dict[str, Any], candidate_paths: list[Path]) -> None:
    ...
    put(
        "artifact_wave02_decision_replay_l5_routing_decision_summary_v0",
        "l5_routing_decision_summary",
        ROUTING_SUMMARY,
        "Wave02 decision replay L5 routing decision summary",
    )
    put(
        "artifact_wave02_decision_replay_l5_routing_decision_index_v0",
        "l5_routing_decision_index",
        ROUTING_INDEX,
        "Wave02 decision replay L5 routing decision row index",
    )
    put(
        "artifact_wave02_decision_replay_l5_routing_decision_closeout_v0",
        "work_closeout",
        ROUTING_CLOSEOUT,
        "Wave02 decision replay L5 routing decision closeout",
    )
...
def update_control_records(summary: dict[str, Any]) -> None:
    ...
    counts["wave02_decision_replay_l5_routing_decision"] = summary["counts"]
    ...
    if (REPO_ROOT / GOAL_REGISTRY).exists():
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                row["claim_boundary"] = "active_goal_wave02_l5_candidate_runtime_evidence_preparation_not_goal_achieve"
        if goal_rows:
...
def writer_scope_self_check(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    for artifact_id, path in [
        ("artifact_wave02_decision_replay_l5_routing_decision_summary_v0", ROUTING_SUMMARY),
        ("artifact_wave02_decision_replay_l5_routing_decision_index_v0", ROUTING_INDEX),
        ("artifact_wave02_decision_replay_l5_routing_decision_closeout_v0", ROUTING_CLOSEOUT),
    ]:
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/decide_wave02_tradeability_decision_replay_l5_routing.py"]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Decide Wave02 decision replay L5 routing and open candidate manifests.")

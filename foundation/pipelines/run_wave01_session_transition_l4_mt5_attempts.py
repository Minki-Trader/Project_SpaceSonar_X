...
SUBWORK_ID = "work_wave01_session_transition_l4_mt5_runtime_execution_v0"
CAMPAIGN_ID = "campaign_us100_session_transition_regime_surface_v0"
SWEEP_ID = "sweep_us100_session_transition_broad_v0"
SUMMARY_ID = "wave01_session_transition_l4_runtime_execution_summary_v0"
CLAIM_BOUNDARY = (
    "wave01_session_transition_l4_score_runtime_observation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate"
)
SUMMARY_CLAIM_BOUNDARY = (
    "wave01_session_transition_l4_runtime_execution_progress_only_"
    "no_runtime_authority_no_economics_pass_no_candidate"
)
...
CLOSEOUT_PATH = Path(
    ...
    "work_wave01_session_transition_l4_mt5_runtime_execution_v0_closeout.yaml"
)
...
def normalize_compile_summary(repo_root: Path, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["version"] = "wave01_session_transition_l4_runtime_execution_compile_summary_v1"
    normalized["summary_id"] = "wave01_session_transition_l4_runtime_execution_compile_summary_v0"
    ...
    base.write_yaml(repo_root / OUTPUT_DIR / "l4_runtime_execution_compile_summary.yaml", normalized)
    return normalized
...
def normalize_attempt_outputs(repo_root: Path, row: dict[str, str], execution_row: dict[str, Any]) -> dict[str, Any]:
    ...
    if terminal_path.exists():
        ...
        terminal["version"] = "wave01_session_transition_l4_terminal_run_summary_v1"
    ...
    if score_path.exists():
        ...
        score["version"] = "wave01_session_transition_l4_score_telemetry_summary_v1"
        ...
        score["claim_boundary"] = CLAIM_BOUNDARY if execution_row.get("telemetry_observed") else score.get("claim_boundary")
    ...
    routing["routing_scope"] = "wave01_session_transition_l4_split_runtime_score_probe_execution"
    ...
    memory = "Wave01 L4 runner normalizes reused Wave0 runner outputs to prevent campaign identity drift."
    if memory not in prevention:
        prevention.append(memory)
    adapter_memory = (
        "Wave01 session-transition L4 uses EA-side New York/session/local feature reconstruction; "
        "compare proxy/runtime rows before judging score drift."
    )
    if adapter_memory not in prevention:
        prevention.append(adapter_memory)
    time_memory = (
        "InpBarTimeToUtcOffsetHours defaults to 0 for the session-transition adapter; any broker-time "
        "to UTC binding change must be recorded per attempt before parity claims."
    )
    if time_memory not in prevention:
        prevention.append(time_memory)
...
def normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    summary["version"] = "wave01_session_transition_l4_runtime_execution_summary_v1"
    ...
    summary["judgment"]["next_action"] = (
        "aggregate paired Wave01 validation/research_oos L4 telemetry and decide L5 routing"
        if summary["judgment"]["runtime_probe_completed_for_all_prepared_attempts"]
        else "continue running remaining prepared Wave01 L4 Strategy Tester attempts"
    )
    memory = "Wave01 runtime execution uses Wave01-specific control records; reused Wave0 helper output is normalized before closeout."
    prevention = summary.setdefault("prevention_memory", [])
    if memory not in prevention:
        prevention.append(memory)
    return summary
...
def build_closeout(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    if summary["status"] == base.PARTIAL_STATUS:
        missing.append("remaining_prepared_L4_attempts")
    else:
        missing.append("paired_L4_period_aggregation_pending")
    return {
...
def upsert_artifact_registry(repo_root: Path, summary: dict[str, Any], execution_rows: list[dict[str, Any]]) -> None:
    ...
    for artifact_id, artifact_type, path, notes in [
        (
            "artifact_wave01_session_transition_l4_runtime_execution_summary_v0",
            "l4_runtime_execution_summary",
            RUNTIME_SUMMARY,
            "Wave01 session-transition L4 runtime execution progress summary",
        ),
        (
            "artifact_wave01_session_transition_l4_runtime_execution_index_v0",
            "l4_runtime_execution_index",
            RUNTIME_INDEX,
            "Wave01 session-transition L4 terminal execution index",
        ),
        (
            "artifact_wave01_session_transition_l4_runtime_execution_closeout_v0",
            "work_closeout",
            CLOSEOUT_PATH,
            "Wave01 session-transition L4 runtime execution subwork closeout",
        ),
        (
            "artifact_wave01_session_transition_l4_runtime_compile_summary_v0",
            "l4_runtime_compile_summary",
            Path(summary["compile_summary"]["path"]),
            "EA binary availability check for Wave01 session-transition L4 execution",
        ),
        (
            "artifact_wave01_session_transition_l4_score_probe_ea_source_v0",
            "mt5_ea_source",
            base.EA_SOURCE,
            "non-trading L4 score telemetry probe source reused for Wave01",
        ),
    ]:
        put(
            {
    ...
    if (repo_root / base.EA_BINARY).exists():
        put(
            {
                "artifact_id": "artifact_wave01_session_transition_l4_score_probe_ea_binary_v0",
    ...
    for row in execution_rows:
        ...
        for suffix, artifact_type, path, availability, notes in [
            (
                ...
                "Wave01 attempt manifest updated with L4 terminal execution evidence",
            ),
            (
                "tester_config",
                "tester_config",
                ...
                "present_hash_recorded",
                "Wave01 tester config used for terminal execution",
            ),
            (
                "terminal_summary",
                "terminal_run_summary",
                Path(row["terminal_run_summary_path"]),
                "present_hash_recorded",
                "Wave01 terminal launch and mode evidence",
            ),
            (
                "score_telemetry_summary",
                "score_telemetry_summary",
                Path(row["score_telemetry_summary_path"]),
                "present_hash_recorded",
                "Wave01 score telemetry summary",
            ),
        ]:
            put(
                {
            ...
            put(
                {
                    ...
                    "consumer": WORK_ITEM_ID,
                    "claim_boundary": row["claim_boundary"],
                    "notes": "raw Wave01 score telemetry is local/generated; committed summary is the indexable evidence",
                }
            )
...
def update_control_records(repo_root: Path, summary: dict[str, Any]) -> None:
    ...
    current_truth["wave01_session_transition_l4_runtime_execution_summary"] = RUNTIME_SUMMARY.as_posix()
    current_truth["wave01_session_transition_l4_runtime_execution_status"] = summary["status"]
    current_truth["wave01_session_transition_l4_runtime_execution_counts"] = summary["counts"]
    if summary["status"] == base.PARTIAL_STATUS:
        next_work["status"] = "l4_strategy_tester_execution_in_progress"
        next_work["missing_material_if_relevant"] = ["remaining_prepared_L4_attempts"]
    else:
        next_work["status"] = "l4_strategy_tester_execution_completed_for_prepared_attempts"
        next_work["missing_material_if_relevant"] = ["paired_L4_period_aggregation_pending"]
    ...
    phase = (
        "wave01_session_transition_l4_terminal_execution_in_progress"
        if summary["status"] == base.PARTIAL_STATUS
        else "wave01_session_transition_l4_pair_judgment_required_next"
    )
    ...
    session_transition["l4_runtime_execution_summary"] = RUNTIME_SUMMARY.as_posix()
    session_transition["l4_runtime_execution_status"] = summary["status"]
    session_transition["l4_runtime_execution_counts"] = summary["counts"]
    ...
    claims["wave0_third_campaign_L4_status"] = (
        "L4_terminal_execution_in_progress"
        if summary["status"] == base.PARTIAL_STATUS
        else "L4_terminal_execution_attempted_pair_judgment_required"
    )
    claims["wave0_third_campaign_l4_runtime_execution_summary"] = RUNTIME_SUMMARY.as_posix()
    claims["wave0_third_campaign_l4_runtime_execution_status"] = summary["status"]
    claims["wave0_third_campaign_l4_runtime_execution_counts"] = summary["counts"]
    ...
    if (repo_root / GOAL_REGISTRY).exists():
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                row["claim_boundary"] = "active_goal_wave01_session_transition_l4_runtime_execution_not_goal_achieve"
        if goal_rows:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prepared Wave01 session-transition L4 MT5 Strategy Tester attempts.")
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/run_wave01_session_transition_l4_mt5_attempts.py"]

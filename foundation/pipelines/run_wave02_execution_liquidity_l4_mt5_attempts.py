...
WORK_ITEM_ID = "work_wave02_execution_liquidity_l4_runtime_execution_v0"
SUBWORK_ID = "work_wave02_execution_liquidity_l4_strategy_tester_execution_v0"
...
SUMMARY_ID = "wave02_execution_liquidity_l4_runtime_execution_summary_v0"
CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l4_score_runtime_observation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
SUMMARY_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l4_runtime_execution_progress_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)

OUTPUT_DIR = Path("lab/campaigns/campaign_us100_wave02_execution_liquidity_surface_v0/l4_follow_through")
...
CLOSEOUT_PATH = Path(
    ...
    "work_wave02_execution_liquidity_l4_strategy_tester_execution_v0_closeout.yaml"
)
...
def normalize_attempt_outputs(repo_root: Path, row: dict[str, str], execution_row: dict[str, Any]) -> dict[str, Any]:
    ...
    if terminal_path.exists():
        ...
        terminal["version"] = "wave02_execution_liquidity_l4_terminal_run_summary_v1"
    ...
    if score_path.exists():
        ...
        score["version"] = "wave02_execution_liquidity_l4_score_telemetry_summary_v1"
    ...
    if diagnostic_path.exists():
        ...
        diagnostic["version"] = "wave02_execution_liquidity_l4_score_diagnostic_summary_v1"
    ...
    manifest["writer_owned_outputs"] = [
        ...
        score_path.relative_to(repo_root).as_posix(),
    ...
    routing["primary_family"] = PRIMARY_FAMILY
    routing["primary_skill"] = PRIMARY_SKILL
    routing["support_skills"] = ["spacesonar-evidence-provenance", "spacesonar-claim-discipline"]
    routing["routing_scope"] = "wave02_execution_liquidity_l4_split_runtime_score_probe_execution"
    ...
    memory = (
        "Wave02 execution/liquidity L4 runner normalizes reused score-probe helper outputs to prevent "
    ...
    parity["follow_up_action"] = manifest.get("next_action", "continue Wave02 execution/liquidity L4 period-role execution")
...
def normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    summary["version"] = "wave02_execution_liquidity_l4_runtime_execution_summary_v1"
    ...
    summary["artifact_outputs"]["attempt_tester_report_receipts"] = [
        (Path(path).parent / "tester_report_receipt.yaml").as_posix()
        for path in summary.get("artifact_outputs", {}).get("attempt_terminal_summaries", [])
    ]
    summary["judgment"]["next_action"] = (
        "aggregate paired Wave02 execution/liquidity validation/research_oos L4 telemetry before any L5 candidate routing"
        if summary["judgment"]["runtime_probe_completed_for_all_prepared_attempts"]
        else "continue running remaining prepared Wave02 execution/liquidity L4 Strategy Tester attempts"
    )
    summary["unresolved_blockers_or_none"] = (
        ["Wave02_execution_liquidity_L4_pair_judgment_pending"]
        if summary["judgment"]["runtime_probe_completed_for_all_prepared_attempts"]
        else ["L4_split_runtime_probe_terminal_execution_pending"]
    )
    ...
    summary["writer_scope_self_check"] = {
        ...
        "rule": "runtime writer rewrites final summary and closeout after declared outputs exist",
    }
    summary.setdefault("prevention_memory", []).append(
        "Wave02 execution/liquidity runtime execution uses ELQ-specific IDs and claim boundaries while reusing the score-probe helper."
    )
    summary.setdefault("try_first_disposition", {})["policy_applied"] = (
        "missing Wave02 execution/liquidity score-probe runner entrypoint was repaired before blocked/deferred/invalid disposition"
    )
    return summary
...
def upsert_artifact_registry(repo_root: Path, summary: dict[str, Any], execution_rows: list[dict[str, Any]]) -> None:
    ...
    for artifact_id, artifact_type, path, notes in [
        (
            "artifact_wave02_execution_liquidity_l4_runtime_execution_summary_v0",
            "l4_runtime_execution_summary",
            RUNTIME_SUMMARY,
            "Wave02 execution/liquidity L4 runtime execution progress summary",
        ),
        (
            "artifact_wave02_execution_liquidity_l4_runtime_execution_index_v0",
            "l4_runtime_execution_index",
            RUNTIME_INDEX,
            "Wave02 execution/liquidity L4 terminal execution index",
        ),
        (
            "artifact_wave02_execution_liquidity_l4_runtime_execution_closeout_v0",
            "work_closeout",
            CLOSEOUT_PATH,
            "Wave02 execution/liquidity L4 runtime execution subwork closeout",
        ),
        (
            "artifact_wave02_execution_liquidity_l4_runtime_compile_summary_v0",
            "l4_runtime_compile_summary",
            Path(summary["compile_summary"]["path"]),
            "EA binary availability check for Wave02 execution/liquidity L4 execution",
        ),
        (
            "artifact_wave02_execution_liquidity_l4_score_probe_ea_source_v0",
            "mt5_ea_source",
            base.EA_SOURCE,
    ...
    if (repo_root / base.EA_BINARY).exists():
        put(
            {
                "artifact_id": "artifact_wave02_execution_liquidity_l4_score_probe_ea_binary_v0",
...
def update_control_records(repo_root: Path, summary: dict[str, Any]) -> None:
    ...
    next_work["unresolved_blockers"] = (
        ...
        if summary["status"] == base.PARTIAL_STATUS
        else ["Wave02_execution_liquidity_L4_pair_judgment_pending"]
    )
    next_work["reopen_conditions"] = (
        ["portable Strategy Tester execution records telemetry and completed report hashes"]
        if summary["status"] == base.PARTIAL_STATUS
        else ["write Wave02 execution/liquidity l4_pair_judgment_summary and l4_pair_judgment_index before L5 routing"]
    )
    next_work["next_action"] = summary["judgment"]["next_action"]
    next_work["writer_contract_version"] = WRITER_CONTRACT_VERSION
    next_work["primary_family"] = PRIMARY_FAMILY
    next_work["primary_skill"] = PRIMARY_SKILL
    next_work["source_of_truth_paths"] = [
        RUNTIME_SUMMARY.as_posix(),
        RUNTIME_INDEX.as_posix(),
    ]
    next_work["writer_owned_outputs"] = summary["writer_owned_outputs"]
    next_work["validation_depth"] = VALIDATION_DEPTH
    next_work["non_pytest_smokes"] = list(NON_PYTEST_SMOKES)
    next_work["skipped_broad_validations"] = list(SKIPPED_BROAD_VALIDATIONS)
    next_work["broad_validation_escalation_reason"] = BROAD_VALIDATION_ESCALATION_REASON
    next_work["writer_scope_self_check"] = summary.get("writer_scope_self_check", {})
    next_work["forbidden_claims"] = list(FORBIDDEN_CLAIMS)
    next_work["unresolved_blockers_or_none"] = next_work["unresolved_blockers"]
    next_work["next_action_or_reopen_condition"] = summary["judgment"]["next_action"]
    ...
    phase = (
        "wave02_execution_liquidity_l4_terminal_execution_in_progress"
        if summary["status"] == base.PARTIAL_STATUS
        else "wave02_execution_liquidity_l4_pair_judgment_required_next"
    )
    campaign_status = (
        "wave02_execution_liquidity_l4_runtime_execution_in_progress"
        if summary["status"] == base.PARTIAL_STATUS
        else "wave02_execution_liquidity_l4_runtime_execution_completed_pair_judgment_next"
    )
    ...
    workspace["summary_counts"]["wave02_execution_liquidity_l4_runtime_execution"] = {
        **counts_payload(summary),
        "candidate_count": 0,
        "l5_candidate_count": 0,
    }
    materialization = workspace.setdefault("wave02_execution_liquidity_l4_materialization", {})
...
def build_writer_scope_self_check(
    repo_root: Path,
    ...
    return {
        "status": "passed" if not missing and forbidden_claims_respected else "failed",
        ...
        "forbidden_claims_respected": forbidden_claims_respected,
        "runtime_probe_complete": bool(completion.get("runtime_probe_complete")),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prepared Wave02 execution/liquidity L4 MT5 Strategy Tester attempts.")
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/run_wave02_execution_liquidity_l4_mt5_attempts.py"]

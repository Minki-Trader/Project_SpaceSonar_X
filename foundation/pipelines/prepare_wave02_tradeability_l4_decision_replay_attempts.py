...
PARENT_WORK_ITEM_ID = "work_wave02_tradeability_l5_routing_decision_v0"
WORK_ITEM_ID = "work_wave02_tradeability_l4_decision_replay_adapter_preparation_v0"
NEXT_WORK_ITEM_ID = "work_wave02_tradeability_decision_replay_runtime_execution_v0"

OUTPUT_DIR = Path("lab/campaigns/campaign_us100_wave02_tradeability_decision_surface_v0/l4_follow_through/decision_replay")
...
PAIR_INDEX = Path("lab/campaigns/campaign_us100_wave02_tradeability_decision_surface_v0/l4_follow_through/l4_pair_judgment_index.csv")
SOURCE_PREP_INDEX = Path(
    "lab/campaigns/campaign_us100_wave02_tradeability_decision_surface_v0/l4_follow_through/l4_attempt_preparation_index.csv"
)
CLOSEOUT_PATH = Path(
    "lab/goals/goal_us100_onnx_forward_boundary_v0/"
    "work_wave02_tradeability_l4_decision_replay_adapter_preparation_v0_closeout.yaml"
)
...
COMMON_DECISION_ROOT = "SpaceSonar\\wave02_tradeability_l4_decision_replay"

CLAIM_BOUNDARY = (
    "wave02_decision_replay_adapter_preparation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_decision_replay_runtime_execution_pending_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
STATUS = "wave02_decision_replay_adapter_attempts_prepared_terminal_execution_next"
NEXT_ACTION = "execute prepared Wave02 decision replay MT5 attempts with portable terminal only, then judge paired validation/research_oos results"
FORBIDDEN_CLAIMS = [
    "selected_baseline",
    "operating_reference",
    "operating_promotion",
...
def build_tester_config_text(
    *,
    ...
    lines = [
        "; SpaceSonar Wave02 tradeability L4 decision replay probe.",
    ...
    *,
    ...
    return {
        ...
        "runtime_learning_probe_decision": {
            ...
            "reason": (
                "Wave02 preserved L4 tradeability score-observation pairs require a sparse decision replay "
        ...
        "runtime_surface_contract": {
            ...
            "surface_scope": "wave02_tradeability_sparse_decision_replay_from_mt5_score_telemetry",
        ...
        "proxy_runtime_parity": {
            ...
            "known_differences": [
                ...
                "Wave02 tradeability score is direction-agnostic; direction is supplied by a declared momentum_ret_1 policy.",
        ...
        "required_gate_coverage": {
            "passed": [
                ...
                "wave02_tradeability_decision_adapter_prepared",
...
def build_records(created_at_utc: str, direction_policy: str) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], dict[str, dict[str, Any]], dict[str, str]]:
    pair_rows = [
        row
        for row in read_csv_rows(REPO_ROOT / PAIR_INDEX)
        if row.get("proxy_judgment") == "preserved_clue"
        and row.get("l5_routing_status") == "l5_decision_review_required_no_candidate_claim"
    ]
    ...
    for pair in sorted(pair_rows, key=lambda item: item["cell_id"]):
        ...
        eligibility_reason = (
            "eligible_direction_agnostic_tradeability_momentum_ret_1_adapter"
            if eligible and execution_kind == "direction_policy_required"
            else "eligible_direct_trade_adapter"
            if eligible
        ...
        if eligible:
            ...
            for period_role, source_attempt_id in [
                ...
                row: dict[str, Any] = {
                    ...
                    "decision_output": "score_ge_high_uses_momentum_ret_1_side_otherwise_flat",
    ...
    summary = {
        "version": "wave02_tradeability_l4_decision_replay_adapter_preparation_summary_v1",
        "summary_id": "wave02_tradeability_l4_decision_replay_adapter_preparation_v0",
        "work_item_id": WORK_ITEM_ID,
        "parent_work_item_id": PARENT_WORK_ITEM_ID,
        ...
        "id_chain": {
            ...
            "sweep_id": SWEEP_ID,
            "run_id": None,
            "artifact_id": "artifact_wave02_tradeability_decision_replay_adapter_prep_summary_v0",
            "bundle_id": None,
            "candidate_id": None,
        },
        "campaign_id": CAMPAIGN_ID,
        ...
        "experiment_design": {
            ...
            "surface_id": SURFACE_ID,
            "sweep_id": SWEEP_ID,
            "hypothesis": "Wave02 tradeability preserved clues may only matter if score-high bars can be translated into sparse side-bearing tester trades.",
            "decision_use": "score_high_tradeability_gate_with_momentum_ret_1_side_policy",
            "comparison_baseline": "Wave02 L4 non-trading score probe pair observations",
            "control_variables": [
                "FPMarkets US100 M5",
            ...
            "reopen_or_stop_condition": "rerun preparation only if pair judgment or source score attempt paths change",
        ...
        "judgment": {
            "result_subject": "Wave02 preserved-clue tradeability decision replay adapter preparation",
        ...
        "environment": {
            "command_argv": ["python", "foundation/pipelines/prepare_wave02_tradeability_l4_decision_replay_attempts.py"],
            "cwd": ".",
...
def build_compile_summary(created_at_utc: str) -> dict[str, Any]:
    return {
        "version": "wave02_decision_replay_adapter_compile_summary_v1",
...
def upsert_artifact_registry(summary: dict[str, Any], attempt_rows: list[dict[str, Any]]) -> None:
    ...
    for artifact_id, artifact_type, path, notes in [
        ("artifact_wave02_tradeability_decision_replay_adapter_prep_summary_v0", "decision_replay_adapter_summary", SUMMARY_PATH, "summary for Wave02 decision replay adapter preparation"),
        ("artifact_wave02_tradeability_decision_replay_adapter_prep_index_v0", "decision_replay_adapter_index", INDEX_PATH, "index of prepared Wave02 decision replay attempts"),
        ("artifact_wave02_tradeability_decision_replay_adapter_eligibility_index_v0", "decision_replay_adapter_eligibility_index", ELIGIBILITY_INDEX_PATH, "eligibility index for Wave02 preserved clue decision replay"),
        ("artifact_wave02_tradeability_decision_replay_adapter_compile_summary_v0", "ea_compile_summary", COMPILE_SUMMARY_PATH, "EA binary/source preflight for decision replay adapter"),
        ("artifact_wave02_tradeability_decision_replay_adapter_prep_closeout_v0", "work_closeout", CLOSEOUT_PATH, "closeout for Wave02 decision replay adapter preparation"),
    ]:
        put(
            {
...
def update_control_records(summary: dict[str, Any]) -> None:
    ...
    input_hashes = [artifact_ref(path) for path in [PAIR_INDEX, SOURCE_PREP_INDEX, EXECUTION_PROFILE, RUNTIME_CONTRACT, PERIOD_PROFILE]]
    next_work = {
        ...
        "summary": "Execute prepared Wave02 sparse decision replay attempts.",
        "provenance": {
            "source": WORK_ITEM_ID,
            "campaign_id": CAMPAIGN_ID,
            "source_of_truth": SUMMARY_PATH.as_posix(),
        ...
        "unresolved_blockers": ["Wave02_decision_replay_terminal_execution_pending"],
        "reopen_conditions": [
            "rerun adapter preparation if Wave02 L4 pair judgment or source score attempt index changes",
            "do not open candidate-specific L5 materialization until decision replay pair judgment exists",
        ],
        "status": "wave02_decision_replay_terminal_execution_pending",
    ...
    resume["cursor_state"] = "wave02_decision_replay_adapter_prepared_terminal_execution_pending"
    resume["active_phase"] = "wave02_decision_replay_adapter_prepared_terminal_execution_pending"
    ...
    resume["unresolved_blockers"] = ["Wave02_decision_replay_terminal_execution_pending"]
    ...
    goal["active_phase"] = "wave02_decision_replay_adapter_prepared_terminal_execution_pending"
    goal["claim_boundary"] = NEXT_CLAIM_BOUNDARY
    goal["next_work_item"] = {
        "work_item_id": NEXT_WORK_ITEM_ID,
        "path": NEXT_WORK_ITEM.as_posix(),
        "summary": "Wave02 decision replay terminal execution pending; no candidate claim.",
    }
    wave02 = goal.setdefault("wave02_tradeability_campaign", {})
    wave02["status"] = "wave02_decision_replay_adapter_prepared_terminal_execution_pending"
    ...
    campaign["status"] = "wave02_decision_replay_adapter_prepared_terminal_execution_pending"
    ...
    workspace["unresolved_blockers"] = ["Wave02_decision_replay_terminal_execution_pending"]
    ...
    counts["wave02_decision_replay_adapter_preparation"] = summary["counts"]
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                if "active_phase" in row:
                    row["active_phase"] = "wave02_decision_replay_adapter_prepared_terminal_execution_pending"
...
def smoke_outputs(summary: dict[str, Any], attempt_rows: list[dict[str, Any]]) -> list[str]:
    ...
    if len(eligibility_rows) != counts.get("preserved_clue_count"):
    ...
    for artifact_id, path in [
        ("artifact_wave02_tradeability_decision_replay_adapter_prep_summary_v0", SUMMARY_PATH),
        ("artifact_wave02_tradeability_decision_replay_adapter_prep_index_v0", INDEX_PATH),
        ("artifact_wave02_tradeability_decision_replay_adapter_eligibility_index_v0", ELIGIBILITY_INDEX_PATH),
        ("artifact_wave02_tradeability_decision_replay_adapter_compile_summary_v0", COMPILE_SUMMARY_PATH),
        ("artifact_wave02_tradeability_decision_replay_adapter_prep_closeout_v0", CLOSEOUT_PATH),
    ]:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare Wave02 tradeability score replay decision MT5 attempts.")

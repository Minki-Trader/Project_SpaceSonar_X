...
import foundation.pipelines.materialize_wave02_execution_liquidity_first_batch_specs as spec_writer
from foundation.features.wave02_execution_liquidity_features import build_wave02_execution_liquidity_features
from foundation.labels.wave02_execution_liquidity_labels import build_wave02_execution_liquidity_labels
...
WORK_ITEM_ID = "work_wave02_execution_liquidity_execute_proxy_batch_v0"
PARENT_WORK_ITEM_ID = spec_writer.WORK_ITEM_ID
NEXT_WORK_ITEM_ID = "work_wave02_execution_liquidity_l4_materialization_preflight_v0"

STATUS = "wave02_execution_liquidity_proxy_batch_executed_l4_required"
RUN_STATUS = "executed_proxy_observation_l4_required"
NEXT_STATUS = "wave02_execution_liquidity_l4_materialization_preflight_pending"
CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_proxy_observation_l4_required_no_candidate_no_runtime_authority_"
    "no_economics_pass_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l4_materialization_pending_no_candidate_no_runtime_authority_"
    "no_economics_pass_no_live_readiness_no_goal_achieve"
)
...
def usable_feature_columns(features: pd.DataFrame, train_mask: pd.Series) -> list[str]:
    ...
    if not columns:
        raise ValueError("no usable Wave02 execution/liquidity feature columns")
    return columns
...
def model_family(axis_values: dict[str, Any]) -> str:
    ...
    if not family:
        raise ValueError(f"unsupported Wave02 execution/liquidity model_variant: {value}")
    return family
...
def run_one(manifest_path: Path, frame: pd.DataFrame, command_argv: list[str]) -> dict[str, Any]:
    ...
    report_path = reports / "proxy_execution_liquidity_report.json"
    ...
    model_validation = {
        ...
        "comparison_baseline": ["no_trade_baseline_reference_only", "closed_wave02_cost_risk_negative_memory_reference_only"],
        "anti_authority_laundering_judgment": "exploratory_proxy_observation_only",
        "validation_judgment": judgment,
    }
    report = {
        "version": "wave02_execution_liquidity_proxy_report_v1",
        "run_id": run_id,
    ...
    lineage = {
        ...
        "producer": {
            "type": "script",
            "identity": "foundation/pipelines/run_wave02_execution_liquidity_proxy_batch.py",
...
def summary_payload(results: list[dict[str, Any]], command_argv: list[str]) -> dict[str, Any]:
    ...
    return {
        "version": "wave02_execution_liquidity_proxy_execution_summary_v1",
        "summary_id": "wave02_execution_liquidity_proxy_execution_summary_v0",
...
def next_work_item() -> dict[str, Any]:
    return {
        ...
        "acceptance_criteria": [
            "materialize ONNX/runtime-follow-through prep for executed execution/liquidity proxy runs",
        ...
        "next_action": "prepare Wave02 execution/liquidity L4 materialization and runtime attempts",
        "current_truth": {
            "proxy_execution_summary": SUMMARY_PATH.as_posix(),
            "proxy_execution_index": INDEX_PATH.as_posix(),
        },
        "source_of_truth_paths": [SUMMARY_PATH.as_posix(), INDEX_PATH.as_posix()],
        "writer_owned_outputs": ["onnx_bundles", "runtime_attempt_manifests", "l4_preflight_summary"],
        "validation_depth": "writer_scope_smoke",
        "non_pytest_smokes": ["py_compile", "writer_scope_evidence_smoke", "active_pointer_smoke", "machine_yaml_identity_lint"],
        ...
        "broad_validation_escalation_reason": "none_l4_materialization_pending_no_protected_claim",
        "writer_scope_self_check": "required_before_close",
        "unresolved_blockers": ["Wave02_execution_liquidity_L4_materialization_not_prepared"],
        "unresolved_blockers_or_none": ["Wave02_execution_liquidity_L4_materialization_not_prepared"],
...
def update_control_records(summary: dict[str, Any], results: list[dict[str, Any]]) -> None:
    ...
    sweep = load_yaml(REPO_ROOT / spec_writer.PATHS["sweep_manifest"])
    sweep["updated_at_utc"] = summary["created_at_utc"]
    sweep["status"] = STATUS
    sweep["claim_boundary"] = NEXT_CLAIM_BOUNDARY
    sweep["executed_proxy_run_count"] = len(results)
    sweep["next_action"] = NEXT_WORK_ITEM_ID
    write_yaml(REPO_ROOT / spec_writer.PATHS["sweep_manifest"], sweep)
    ...
    resume["unresolved_blockers"] = ["Wave02_execution_liquidity_L4_materialization_not_prepared"]
    ...
    goal["next_work_item"] = {
        "work_item_id": NEXT_WORK_ITEM_ID,
        "path": spec_writer.PATHS["next_work_item"].as_posix(),
        "summary": "Wave02 execution/liquidity proxy batch executed; L4 materialization pending.",
    }
    ...
    workspace["unresolved_blockers"] = ["Wave02_execution_liquidity_L4_materialization_not_prepared"]
    workspace.setdefault("summary_counts", {})["wave02_execution_liquidity_proxy_execution"] = summary["counts"]
...
def update_registries(summary: dict[str, Any], results: list[dict[str, Any]]) -> None:
    ...
    upsert_csv_row(
        ...
        {
            "wave_id": WAVE_ID,
            "status": "wave02_campaign_003_proxy_executed_l4_required",
            "created_at_utc": "2026-06-27T12:15:00Z",
            "wave_path": spec_writer.PATHS["wave_allocation"].as_posix(),
            "allocation_goal": "Wave02 execution/liquidity campaign proxy batch executed; L4 follow-through required.",
            "max_runs": "72",
            "claim_boundary": NEXT_CLAIM_BOUNDARY,
    ...
    update_artifact_registry(results)


def update_artifact_registry(results: list[dict[str, Any]]) -> None:
    artifacts = {
        "artifact_wave02_execution_liquidity_proxy_summary_v0": ("proxy_execution_summary", SUMMARY_PATH, ""),
        "artifact_wave02_execution_liquidity_proxy_index_v0": ("proxy_execution_index", INDEX_PATH, ""),
        "artifact_wave02_execution_liquidity_proxy_closeout_v0": ("work_closeout", WORK_CLOSEOUT, ""),
    }
    for result in results:
        run_id = result["run_id"]
        artifacts[f"artifact_{run_id}_manifest_v0"] = ("run_manifest", Path(result["run_manifest_path"]), run_id)
        artifacts[f"artifact_{run_id}_receipt_v0"] = ("experiment_receipt", Path(result["receipt_path"]), run_id)
        artifacts[f"artifact_{run_id}_lineage_v0"] = ("artifact_lineage", Path(result["lineage_path"]), run_id)
        artifacts[f"artifact_{run_id}_metrics_v0"] = ("metrics", Path(result["metrics_path"]), run_id)

    for artifact_id, (artifact_type, path, run_id) in artifacts.items():
        full = REPO_ROOT / path
        upsert_csv_row(
            ...
            "artifact_id",
            {
                ...
                "availability": "present_hash_recorded",
                "producer_command": "python foundation/pipelines/run_wave02_execution_liquidity_proxy_batch.py --write-control-records",
                "regeneration_command": "python foundation/pipelines/run_wave02_execution_liquidity_proxy_batch.py --write-control-records",
                "source_of_truth": path.as_posix() if run_id else SUMMARY_PATH.as_posix(),
                "consumer": NEXT_WORK_ITEM_ID,
                "claim_boundary": CLAIM_BOUNDARY,
...
def write_summary_records(summary: dict[str, Any], results: list[dict[str, Any]]) -> None:
    ...
    write_yaml(
        REPO_ROOT / WORK_CLOSEOUT,
        {
            ...
            "source_of_truth_paths": [SUMMARY_PATH.as_posix(), INDEX_PATH.as_posix()],
            "writer_owned_outputs": [SUMMARY_PATH.as_posix(), INDEX_PATH.as_posix(), WORK_CLOSEOUT.as_posix()],
            "validation_depth": "writer_scope_smoke",
            ...
            "broad_validation_escalation_reason": "none_proxy_execution_no_protected_claim",
            ...
            "forbidden_claims": FORBIDDEN_CLAIMS,
            "unresolved_blockers_or_none": "none_for_proxy_execution_next_work_has_l4_materialization_pending",
...
def writer_scope_self_check(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for result in results:
        ...
        manifest = load_json(REPO_ROOT / result["run_manifest_path"])
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute Wave02 execution/liquidity proxy batch.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    command_argv = ["python", "foundation/pipelines/run_wave02_execution_liquidity_proxy_batch.py"]

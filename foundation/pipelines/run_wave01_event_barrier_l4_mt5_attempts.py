...
SUBWORK_ID = "work_wave01_event_barrier_l4_mt5_runtime_execution_v0"
CAMPAIGN_ID = "campaign_us100_event_barrier_decision_surface_v0"
SWEEP_ID = "sweep_us100_event_barrier_broad_v0"
SUMMARY_ID = "wave01_event_barrier_l4_runtime_execution_summary_v0"
CLAIM_BOUNDARY = (
    "wave01_event_barrier_l4_score_runtime_observation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate"
)
SUMMARY_CLAIM_BOUNDARY = (
    "wave01_event_barrier_l4_runtime_execution_progress_only_"
    "no_runtime_authority_no_economics_pass_no_candidate"
)
...
CLOSEOUT_PATH = Path(
    ...
    "work_wave01_event_barrier_l4_mt5_runtime_execution_v0_closeout.yaml"
)
...
def normalize_compile_summary(repo_root: Path, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["version"] = "wave01_event_barrier_l4_runtime_execution_compile_summary_v1"
    normalized["summary_id"] = "wave01_event_barrier_l4_runtime_execution_compile_summary_v0"
...
def normalize_attempt_outputs(repo_root: Path, row: dict[str, str], execution_row: dict[str, Any]) -> dict[str, Any]:
    ...
    if terminal_path.exists():
        ...
        terminal["version"] = "wave01_event_barrier_l4_terminal_run_summary_v1"
    ...
    if score_path.exists():
        ...
        score["version"] = "wave01_event_barrier_l4_score_telemetry_summary_v1"
    ...
    routing["routing_scope"] = "wave01_event_barrier_l4_split_runtime_score_probe_execution"
...
def normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    summary["version"] = "wave01_event_barrier_l4_runtime_execution_summary_v1"
    ...
    summary.setdefault("prevention_memory", []).append(
        "Wave01 runtime execution uses Wave01-specific control records; reused Wave0 helper output is normalized before closeout."
    )
    return summary
...
def upsert_artifact_registry(repo_root: Path, summary: dict[str, Any], execution_rows: list[dict[str, Any]]) -> None:
    ...
    for artifact_id, artifact_type, path, notes in [
        (
            "artifact_wave01_event_barrier_l4_runtime_execution_summary_v0",
            "l4_runtime_execution_summary",
            RUNTIME_SUMMARY,
            "Wave01 event/barrier L4 runtime execution progress summary",
        ),
        (
            "artifact_wave01_event_barrier_l4_runtime_execution_index_v0",
            "l4_runtime_execution_index",
            RUNTIME_INDEX,
            "Wave01 event/barrier L4 terminal execution index",
        ),
        (
            "artifact_wave01_event_barrier_l4_runtime_execution_closeout_v0",
            "work_closeout",
            CLOSEOUT_PATH,
            "Wave01 event/barrier L4 runtime execution subwork closeout",
        ),
        (
            "artifact_wave01_event_barrier_l4_runtime_compile_summary_v0",
            "l4_runtime_compile_summary",
            Path(summary["compile_summary"]["path"]),
            "EA binary availability check for Wave01 event/barrier L4 execution",
        ),
        (
            "artifact_wave01_event_barrier_l4_score_probe_ea_source_v0",
            "mt5_ea_source",
            base.EA_SOURCE,
...
def update_control_records(repo_root: Path, summary: dict[str, Any]) -> None:
    ...
    current_truth["wave01_event_barrier_l4_runtime_execution_summary"] = RUNTIME_SUMMARY.as_posix()
    current_truth["wave01_event_barrier_l4_runtime_execution_status"] = summary["status"]
    current_truth["wave01_event_barrier_l4_runtime_execution_counts"] = summary["counts"]
    if summary["status"] == base.PARTIAL_STATUS:
    ...
    phase = (
        "wave01_event_barrier_l4_terminal_execution_in_progress"
        if summary["status"] == base.PARTIAL_STATUS
        else "wave01_event_barrier_l4_pair_judgment_required_next"
    )
    ...
    event_barrier["l4_runtime_execution_summary"] = RUNTIME_SUMMARY.as_posix()
    event_barrier["l4_runtime_execution_status"] = summary["status"]
    event_barrier["l4_runtime_execution_counts"] = summary["counts"]
    ...
    claims["wave0_second_campaign_L4_status"] = (
        "L4_terminal_execution_in_progress"
        if summary["status"] == base.PARTIAL_STATUS
        else "L4_terminal_execution_attempted_pair_judgment_required"
    )
    claims["wave0_second_campaign_l4_runtime_execution_summary"] = RUNTIME_SUMMARY.as_posix()
    claims["wave0_second_campaign_l4_runtime_execution_status"] = summary["status"]
    claims["wave0_second_campaign_l4_runtime_execution_counts"] = summary["counts"]
    ...
    if (repo_root / GOAL_REGISTRY).exists():
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                row["claim_boundary"] = "active_goal_wave01_event_barrier_l4_runtime_execution_not_goal_achieve"
        if goal_rows:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prepared Wave01 event/barrier L4 MT5 Strategy Tester attempts.")
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/run_wave01_event_barrier_l4_mt5_attempts.py"]

...
WORK_ITEM_ID = "work_wave02_cost_risk_holding_decision_replay_runtime_execution_v0"
SUBWORK_ID = "work_wave02_cost_risk_holding_decision_replay_strategy_tester_execution_v0"
NEXT_WORK_ID = "work_wave02_cost_risk_holding_decision_replay_judgment_v0"
CAMPAIGN_ID = "campaign_us100_wave02_cost_risk_holding_surface_v0"
SWEEP_ID = "sweep_us100_wave02_cost_risk_holding_broad_v0"
SUMMARY_ID = "wave02_cost_risk_holding_decision_replay_runtime_execution_summary_v0"

CLAIM_BOUNDARY = (
    "wave02_cost_risk_holding_decision_replay_runtime_observation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
SUMMARY_CLAIM_BOUNDARY = (
    "wave02_cost_risk_holding_decision_replay_runtime_execution_progress_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_cost_risk_holding_decision_replay_judgment_pending_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)

PARTIAL_STATUS = "partial_wave02_cost_risk_holding_decision_replay_terminal_execution_started"
ALL_ATTEMPTS_STATUS = "wave02_cost_risk_holding_decision_replay_terminal_execution_attempted_for_all_prepared_attempts"
...
CLOSEOUT_PATH = Path(
    ...
    "work_wave02_cost_risk_holding_decision_replay_strategy_tester_execution_v0_closeout.yaml"
)
...
def parse_tester_config_expert(tester_config: Path) -> str | None:
    for line in tester_config.read_text(encoding="utf-8-sig").splitlines():
...
def normalize_attempt_outputs(repo_root: Path, row: dict[str, str], execution_row: dict[str, Any]) -> dict[str, Any]:
    ...
    if terminal_path.exists():
        ...
        terminal["version"] = "wave02_cost_risk_holding_decision_replay_terminal_run_summary_v1"
        ...
        terminal["claim_boundary"] = (
            ...
            else "wave02_cost_risk_holding_decision_replay_terminal_not_launched_no_runtime_completion_claim"
        )
    ...
    if telemetry_path.exists():
        ...
        telemetry["version"] = "wave02_cost_risk_holding_decision_replay_execution_telemetry_summary_v1"
    ...
    if tester_log_path.exists():
        ...
        tester_log["version"] = "wave02_cost_risk_holding_decision_replay_tester_log_summary_v1"
        ...
        tester_log["claim_boundary"] = "wave02_cost_risk_holding_decision_replay_tester_log_observation_only_no_economics_pass"
    ...
    routing["routing_scope"] = "wave02_cost_risk_holding_l4_score_replay_decision_execution"
    ...
    memory = (
        "Wave02 decision replay runner normalizes reused Wave0 helper outputs to prevent "
    ...
    parity["follow_up_action"] = manifest.get("next_action", "judge paired Wave02 decision replay period roles")
...
def normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    summary["version"] = "wave02_cost_risk_holding_decision_replay_runtime_execution_summary_v1"
    ...
    summary["judgment"]["next_action"] = (
        "judge paired Wave02 decision replay validation/research_oos results before any L5 candidate claim"
        if summary["status"] == ALL_ATTEMPTS_STATUS
        else "continue running remaining prepared Wave02 decision replay Strategy Tester attempts"
    )
    summary.setdefault("prevention_memory", []).append(
        "Wave02 decision replay runtime execution owns Wave02 IDs and records no candidate, no economics pass, no live readiness."
    )
    summary.setdefault("try_first_disposition", {})["policy_applied"] = (
        "missing Wave02 decision replay runtime entrypoint was repaired before blocked/deferred/invalid disposition"
    )
    return summary


def normalize_compile_summary(compile_summary: dict[str, Any]) -> dict[str, Any]:
    compile_summary["version"] = "wave02_cost_risk_holding_decision_replay_runtime_compile_summary_v1"
    ...
    compile_summary["claim_boundary"] = "wave02_cost_risk_holding_decision_replay_ea_compile_or_binary_preflight_only_not_strategy_tester_output"
...
def upsert_artifact_registry(repo_root: Path, summary: dict[str, Any], execution_rows: list[dict[str, Any]]) -> None:
    ...
    for artifact_id, artifact_type, path, notes in [
        (
            "artifact_wave02_cost_risk_holding_decision_replay_runtime_execution_summary_v0",
            "decision_replay_runtime_execution_summary",
            RUNTIME_SUMMARY,
            "Wave02 decision replay runtime execution progress summary",
        ),
        (
            "artifact_wave02_cost_risk_holding_decision_replay_runtime_execution_index_v0",
            "decision_replay_runtime_execution_index",
            RUNTIME_INDEX,
            "Wave02 decision replay terminal execution index",
        ),
        (
            "artifact_wave02_cost_risk_holding_decision_replay_runtime_execution_closeout_v0",
            "work_closeout",
            CLOSEOUT_PATH,
            "Wave02 decision replay runtime execution subwork closeout",
        ),
        (
            "artifact_wave02_cost_risk_holding_decision_replay_runtime_compile_summary_v0",
        ...
        (
            "artifact_wave02_cost_risk_holding_decision_replay_ea_source_v0",
            "mt5_ea_source",
            base.EA_SOURCE,
    ...
    if (repo_root / base.EA_BINARY).exists():
        put(
            {
                "artifact_id": "artifact_wave02_cost_risk_holding_decision_replay_ea_binary_v0",
                ...
                "consumer": WORK_ITEM_ID,
                "claim_boundary": "wave02_cost_risk_holding_decision_replay_ea_compile_or_binary_preflight_only_not_strategy_tester_output",
    ...
    for row in execution_rows:
        ...
        for suffix, artifact_type, path, availability, notes in [
            ("manifest", "attempt_manifest", attempt_root / "attempt_manifest.yaml", "present_hash_recorded", "Wave02 decision replay attempt manifest updated with terminal evidence"),
            ("tester_config", "tester_config", attempt_root / "tester_config.ini", "present_hash_recorded", "Wave02 decision replay tester config used for terminal execution"),
            ("terminal_summary", "terminal_run_summary", Path(row["terminal_run_summary_path"]), "present_hash_recorded", "Wave02 decision replay terminal launch and mode evidence"),
            ...
            ("tester_log_summary", "tester_log_summary", Path(row.get("tester_log_summary_path") or attempt_root / "tester_log_summary.yaml"), "present_hash_recorded", "Wave02 decision replay tester log summary"),
            ("tester_report_receipt", "tester_report_receipt", receipt_path, receipt_availability, "Wave02 decision replay tester report receipt"),
        ]:
            put(
                {
...
def next_work_payload(summary: dict[str, Any]) -> dict[str, Any]:
    if summary["status"] == PARTIAL_STATUS:
        return {
            ...
            "parent_work_item_id": "work_wave02_cost_risk_holding_l4_decision_replay_adapter_preparation_v0",
            ...
            "acceptance_criteria": [
                "execute remaining prepared Wave02 decision replay MT5 attempts",
            ...
            "claim_boundary": summary["claim_boundary"],
            "next_action": summary["judgment"]["next_action"],
            "status": "wave02_cost_risk_holding_decision_replay_terminal_execution_in_progress",
            "current_truth": {
            ...
            "unresolved_blockers": ["remaining_Wave02_decision_replay_terminal_execution_pending"],
    ...
    return {
        ...
        "targets": [RUNTIME_SUMMARY.as_posix(), RUNTIME_INDEX.as_posix()],
        "acceptance_criteria": [
            "judge paired Wave02 decision replay validation/research_oos results",
        ...
        "next_action": "write Wave02 decision replay pair judgment before any candidate-specific L5 material",
        "status": "wave02_cost_risk_holding_decision_replay_pair_judgment_pending",
        "current_truth": {
            "decision_replay_runtime_execution_summary": RUNTIME_SUMMARY.as_posix(),
            "decision_replay_runtime_execution_index": RUNTIME_INDEX.as_posix(),
            "decision_replay_runtime_execution_status": summary["status"],
        ...
        "unresolved_blockers": ["Wave02_decision_replay_pair_judgment_pending"],
...
def update_control_records(repo_root: Path, summary: dict[str, Any]) -> None:
    ...
    counts["candidate_count"] = 0
    counts["l5_candidate_count"] = 0
    counts["wave02_cost_risk_holding_decision_replay_runtime_execution"] = summary["counts"]
    ...
    if (repo_root / GOAL_REGISTRY).exists():
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                row["claim_boundary"] = "active_goal_wave02_cost_risk_holding_decision_replay_runtime_progress_not_goal_achieve"
        if goal_rows:
...
def writer_scope_self_check(repo_root: Path, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    for artifact_id, path in [
        ("artifact_wave02_cost_risk_holding_decision_replay_runtime_execution_summary_v0", RUNTIME_SUMMARY),
        ("artifact_wave02_cost_risk_holding_decision_replay_runtime_execution_index_v0", RUNTIME_INDEX),
        ("artifact_wave02_cost_risk_holding_decision_replay_runtime_execution_closeout_v0", CLOSEOUT_PATH),
    ]:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prepared Wave02 cost/risk/holding decision replay MT5 attempts.")
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/run_wave02_cost_risk_holding_l4_decision_replay_attempts.py"]

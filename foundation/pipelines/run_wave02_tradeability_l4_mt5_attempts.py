...
WORK_ITEM_ID = "work_wave02_tradeability_l4_runtime_execution_v0"
SUBWORK_ID = "work_wave02_tradeability_l4_strategy_tester_execution_v0"
...
SUMMARY_ID = "wave02_tradeability_l4_runtime_execution_summary_v0"
CLAIM_BOUNDARY = (
    "wave02_l4_score_runtime_observation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
SUMMARY_CLAIM_BOUNDARY = (
    "wave02_l4_runtime_execution_progress_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
...
CLOSEOUT_PATH = Path(
    ...
    "work_wave02_tradeability_l4_strategy_tester_execution_v0_closeout.yaml"
)
...
def normalize_attempt_outputs(repo_root: Path, row: dict[str, str], execution_row: dict[str, Any]) -> dict[str, Any]:
    ...
    routing["routing_scope"] = "wave02_tradeability_l4_split_runtime_score_probe_execution"
    ...
    memory = (
        "Wave02 tradeability L4 runner normalizes reused score-probe helper outputs to prevent "
...
def normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    summary["version"] = "wave02_tradeability_l4_runtime_execution_summary_v1"
    ...
    summary["judgment"]["next_action"] = (
        "aggregate paired Wave02 validation/research_oos L4 telemetry before any L5 candidate routing"
        if summary["judgment"]["runtime_probe_completed_for_all_prepared_attempts"]
        else "continue running remaining prepared Wave02 L4 Strategy Tester attempts"
    )
    summary.setdefault("prevention_memory", []).append(
        "Wave02 runtime execution uses Wave02-specific IDs and claim boundaries while reusing the score-probe helper."
    )
    summary.setdefault("try_first_disposition", {})["policy_applied"] = (
        "missing Wave02 score-probe runner entrypoint was repaired before blocked/deferred/invalid disposition"
    )
    return summary
...
def upsert_artifact_registry(repo_root: Path, summary: dict[str, Any], execution_rows: list[dict[str, Any]]) -> None:
    ...
    for artifact_id, artifact_type, path, notes in [
        (
            "artifact_wave02_tradeability_l4_runtime_execution_summary_v0",
            "l4_runtime_execution_summary",
            RUNTIME_SUMMARY,
            "Wave02 tradeability L4 runtime execution progress summary",
        ),
        (
            "artifact_wave02_tradeability_l4_runtime_execution_index_v0",
            "l4_runtime_execution_index",
            RUNTIME_INDEX,
            "Wave02 tradeability L4 terminal execution index",
        ),
        (
            "artifact_wave02_tradeability_l4_runtime_execution_closeout_v0",
            "work_closeout",
            CLOSEOUT_PATH,
            "Wave02 tradeability L4 runtime execution subwork closeout",
        ),
        (
            "artifact_wave02_tradeability_l4_runtime_compile_summary_v0",
            "l4_runtime_compile_summary",
            Path(summary["compile_summary"]["path"]),
            "EA binary availability check for Wave02 tradeability L4 execution",
        ),
        (
            "artifact_wave02_tradeability_l4_score_probe_ea_source_v0",
            "mt5_ea_source",
            base.EA_SOURCE,
    ...
    if (repo_root / base.EA_BINARY).exists():
        put(
            {
                "artifact_id": "artifact_wave02_tradeability_l4_score_probe_ea_binary_v0",
...
def update_control_records(repo_root: Path, summary: dict[str, Any]) -> None:
    ...
    resume["next_action"] = WORK_ITEM_ID
    ...
    phase = (
        "wave02_l4_terminal_execution_in_progress"
        if summary["status"] == base.PARTIAL_STATUS
        else "wave02_l4_pair_judgment_required_next"
    )
    ...
    wave02["l4_runtime_execution_counts"] = summary["counts"]
    ...
    if (repo_root / GOAL_REGISTRY).exists():
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                row["claim_boundary"] = "active_goal_wave02_l4_runtime_execution_not_goal_achieve"
        if goal_rows:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prepared Wave02 tradeability L4 MT5 Strategy Tester attempts.")
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/run_wave02_tradeability_l4_mt5_attempts.py"]

...
PARENT_WORK_ITEM_ID = "work_wave02_cost_risk_holding_l4_pair_judgment_v0"
WORK_ITEM_ID = "work_wave02_cost_risk_holding_l5_routing_decision_v0"
NEXT_WORK_ITEM_ID = "work_wave02_cost_risk_holding_l4_decision_replay_adapter_preparation_v0"

OUTPUT_DIR = Path("lab/campaigns/campaign_us100_wave02_cost_risk_holding_surface_v0/l4_follow_through/decision_replay")
PAIR_SUMMARY = Path("lab/campaigns/campaign_us100_wave02_cost_risk_holding_surface_v0/l4_follow_through/l4_pair_judgment_summary.yaml")
...
ROUTING_CLOSEOUT = Path(
    ...
    "work_wave02_cost_risk_holding_l5_routing_decision_v0_closeout.yaml"
)
...
CLAIM_BOUNDARY = (
    "wave02_cost_risk_holding_l5_routing_decision_only_"
    "no_selected_baseline_no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_cost_risk_holding_decision_replay_adapter_preparation_pending_"
    "no_selected_baseline_no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
STATUS = "wave02_cost_risk_holding_l5_routing_decision_completed_adapter_preparation_pending"
NEXT_STATUS = "wave02_cost_risk_holding_decision_replay_adapter_preparation_pending"
NEXT_ACTION = (
    "prepare Wave02 cost/risk/holding sparse score-band decision replay attempts "
...
def route_row(row: dict[str, str]) -> tuple[str, str]:
    if row.get("l5_routing_status") == REQUIRED_SOURCE_L5_STATUS:
        return (
            ...
            "score_probe_preserved_clue_non_trading_requires_sparse_score_band_adapter",
        )
    return (
...
def build_summary(routing_rows: list[dict[str, Any]], started_at_utc: str, command_argv: list[str]) -> dict[str, Any]:
    ...
    return {
        "version": "wave02_cost_risk_holding_l5_routing_decision_summary_v1",
        "summary_id": "wave02_cost_risk_holding_l5_routing_decision_summary_v0",
        ...
        "id_chain": {
            ...
            "sweep_id": SWEEP_ID,
            "run_id": None,
            "artifact_id": "artifact_wave02_cost_risk_holding_l5_routing_decision_summary_v0",
            "bundle_id": None,
            "candidate_id": None,
        },
        "campaign_id": CAMPAIGN_ID,
        ...
        "judgment": {
            "result_subject": "Wave02 cost/risk/holding L5 routing decision from paired L4 score observations",
            ...
            "comparison_baseline": "Wave02 CRH L4 score-pair judgment",
            "tested_factor": "whether preserved CRH score probes justify candidate opening or bounded decision-execution adapter work",
            "kpi_interpretation": "no economics KPI is claimable because the source evidence is score telemetry only",
            "directional_effect_hypothesis": "all six preserved score clues require sparse score-band decision replay before candidate-specific L5 evidence",
        ...
        "prevention_memory": [
            "CRH score-probe L4 observations cannot open candidate manifests by themselves.",
            "Adapter preparation is a runtime follow-through step, not runtime authority or economics pass.",
            "If sparse decision replay is negative, record negative memory before rotating surface or repairing execution semantics.",
        ],
        "unresolved_blockers": ["Wave02_cost_risk_holding_decision_replay_adapter_preparation_pending"],
        "reopen_conditions": [
            "rerun routing decision if CRH L4 pair judgment summary or index changes",
...
def next_work_payload(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "acceptance_criteria": [
            "prepare sparse score-band decision replay attempts for eligible CRH validation/research_oos pairs",
        ...
        "outputs": [
            "lab/campaigns/campaign_us100_wave02_cost_risk_holding_surface_v0/l4_follow_through/decision_replay/adapter_prep_summary.yaml",
        ...
        "summary": "Prepare Wave02 CRH sparse score-band decision replay attempts; no candidate claim.",
        "provenance": {
            "source": WORK_ITEM_ID,
            "campaign_id": CAMPAIGN_ID,
...
def upsert_artifact_registry(summary: dict[str, Any]) -> None:
    ...
    put(
        ...
        ROUTING_SUMMARY,
        "Wave02 CRH L5 routing decision summary; adapter prep next, no candidate",
    )
    put(
        "artifact_wave02_cost_risk_holding_l5_routing_decision_index_v0",
        "l5_routing_decision_index",
        ROUTING_INDEX,
        "Wave02 CRH L5 routing decision row index",
    )
    put(
        "artifact_wave02_cost_risk_holding_l5_routing_decision_closeout_v0",
        "work_closeout",
        ROUTING_CLOSEOUT,
        "closeout for Wave02 CRH L5 routing decision",
    )
...
def update_control_records(summary: dict[str, Any]) -> None:
    ...
    goal["next_work_item"] = {
        "work_item_id": NEXT_WORK_ITEM_ID,
        "path": NEXT_WORK_ITEM.as_posix(),
        "summary": "Wave02 CRH decision replay adapter preparation pending; no candidate claim.",
    }
    ...
    counts["wave02_cost_risk_holding_l5_routing_decision"] = summary["counts"]
    crh = workspace.setdefault("wave02_cost_risk_holding_l4_materialization", {})
    crh["status"] = STATUS
    crh["claim_boundary"] = NEXT_CLAIM_BOUNDARY
    crh["l5_routing_decision_summary"] = ROUTING_SUMMARY.as_posix()
    crh["l5_routing_decision_index"] = ROUTING_INDEX.as_posix()
    crh["l5_routing_decision_status"] = summary["status"]
    crh["l5_routing_decision_counts"] = summary["counts"]
...
def smoke_outputs(summary: dict[str, Any], routing_rows: list[dict[str, Any]]) -> list[str]:
    errors: list[str] = []
    ...
    for artifact_id, path in [
        ("artifact_wave02_cost_risk_holding_l5_routing_decision_summary_v0", ROUTING_SUMMARY),
        ("artifact_wave02_cost_risk_holding_l5_routing_decision_index_v0", ROUTING_INDEX),
        ("artifact_wave02_cost_risk_holding_l5_routing_decision_closeout_v0", ROUTING_CLOSEOUT),
    ]:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Decide Wave02 CRH L5 routing without opening candidate manifests.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    command_argv = [
        ...
        "foundation/pipelines/decide_wave02_cost_risk_holding_l5_routing.py",
        *(argv or []),
    ]
    if args.smoke_only:
    ...
    if errors:
        ...
        return 1
    print(
        "wave02 CRH l5 routing writer-smoke passed: "

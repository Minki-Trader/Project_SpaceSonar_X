...
from foundation.evaluation.kpi_ledger_builder import build_campaign_kpi_ledger_from_existing_evidence  # noqa: E402
from foundation.evaluation.kpi_record_model import DEFAULT_CLAIM_BOUNDARY as KPI_CLAIM_BOUNDARY  # noqa: E402
...
WORK_ITEM_ID = "work_wave03_bounded_synthesis_special_mixing_mix3_proxy_execution_v0"
NEXT_WORK_ITEM_ID = "work_wave03_bounded_synthesis_special_mixing_mix3_l4_materialization_preflight_v0"
...
STATUS = "wave03_bounded_synthesis_mix3_proxy_observation_l4_required"
RUN_STATUS = "executed_mix3_proxy_observation_l4_required"
CLAIM_BOUNDARY = (
    "wave03_bounded_synthesis_mix3_proxy_observation_l4_required_no_candidate_no_selected_baseline_"
    "no_runtime_authority_no_economics_pass_no_live_readiness_no_goal_achieve"
)
NEXT_ACTION = "materialize bounded synthesis mix-3 L4 follow-through preflight"
ENTRYPOINT = "foundation/pipelines/run_wave03_bounded_synthesis_mix3_proxy_batch.py"
...
RUN_REFS = CAMPAIGN_DIR / "mix_specs" / "mix3_run_refs.csv"
RUN_SPECS_MANIFEST = CAMPAIGN_DIR / "mix_specs" / "mix3_run_specs_manifest.yaml"
RUN_SPECS_INDEX = CAMPAIGN_DIR / "mix_specs" / "mix3_run_specs_index.csv"
MIX_MATRIX = CAMPAIGN_DIR / "mix_specs" / "mix3_matrix.csv"
MIX_QUEUE = CAMPAIGN_DIR / "synthesis" / "mix_queue.yaml"
SUMMARY_PATH = CAMPAIGN_DIR / "mix_specs" / "mix3_proxy_execution_summary.yaml"
INDEX_PATH = CAMPAIGN_DIR / "mix_specs" / "mix3_proxy_execution_index.csv"
KPI_SUMMARY_PATH = CAMPAIGN_DIR / "kpi" / "kpi_summary.yaml"
KPI_LEDGER_MANIFEST_PATH = CAMPAIGN_DIR / "kpi" / "kpi_ledger_manifest.yaml"
...
WORK_CLOSEOUT = GOAL_DIR / "work_wave03_bounded_synthesis_special_mixing_mix3_proxy_execution_v0_closeout.yaml"
...
MISSING_EVIDENCE_AFTER_PROXY = [
    "mix3_ONNX_exports_absent",
    "mix3_L4_split_runtime_probe_absent",
    "mix3_MT5_runtime_kpi_absent_until_L4",
    "mix3_proxy_MT5_comparison_absent_until_L4",
...
def validate_declared_inputs(run_refs: list[dict[str, str]]) -> None:
    if len(run_refs) != EXPECTED_RUN_COUNT:
        raise ValueError(f"mix-3 proxy batch requires {EXPECTED_RUN_COUNT} run refs, observed {len(run_refs)}")
    ...
    if declared != observed:
        raise ValueError("mix3_run_refs.csv run order does not match mix3_run_specs_manifest.yaml")

    matrix_fields, matrix_rows = base.read_csv_rows(repo_path(MIX_MATRIX))
    if "run_id" not in matrix_fields:
        raise ValueError("mix3_matrix.csv is missing run_id")
    matrix_run_ids = [row["run_id"] for row in matrix_rows]
    if matrix_run_ids != observed:
        raise ValueError("mix3_matrix.csv run order does not match mix3_run_refs.csv")
...
def summary_payload(results: list[dict[str, str]], command_argv: list[str], created_at: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "version": "wave03_bounded_synthesis_mix3_proxy_execution_summary_v1",
        "summary_id": "wave03_bounded_synthesis_mix3_proxy_execution_summary_v0",
        ...
        "budget_or_environment_blocker": {
            ...
            "reason": "mix-3 is capped to six predeclared specs by the special_mixing cadence gate, not a tiny validation sample",
    ...
    payload.update(
        writer_fields(
            ...
            progress_effect="bounded_synthesis_mix3_proxy_batch_executed_l4_required_next",
            experiment_or_boundary_effect="six_declared_mix3_proxy_runs_executed_without_candidate_or_runtime_claim",
        )
    )
    return payload
...
def next_work_item_payload(summary: dict[str, Any]) -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "acceptance_criteria": [
            "materialize ONNX/runtime-follow-through prep for the six executed mix-3 proxy runs",
        ...
        "outputs": [
            ...
            "runtime/packages/pending_wave03_mix3_l4_bundle_id/experiment_bundle.json",
            "runtime/mt5_attempts/pending_wave03_mix3_l4_attempt_id/attempt_manifest.yaml",
        ...
        "unresolved_blockers": [
            "mix3_l4_follow_through_not_materialized_yet",
            "mix3_runtime_kpi_and_proxy_mt5_comparison_pending_until_L4",
        ],
        "reopen_conditions": [
            "rerun mix-3 proxy batch only if source run specs, row membership, feature/label builders, or decision recipe contract changes",
            "do not close bounded synthesis until mix-3 L4 follow-through, KPI triad, and intent parity are recorded or explicitly reopened",
        ],
    }
    payload.update(
        writer_fields(
            ...
            progress_effect="active_pointer_moved_to_mix3_l4_materialization_preflight",
...
def write_summary_records(summary: dict[str, Any], results: list[dict[str, str]]) -> None:
    ...
    closeout.update(
        writer_fields(
            ...
            progress_effect="bounded_synthesis_mix3_proxy_execution_closeout_recorded",
            experiment_or_boundary_effect="mix3_proxy_batch_completed_with_l4_required_next",
        )
    )
    ...
    row.update(
        {
            ...
            "notes": "Mix-3 proxy batch executed; L4 follow-through and KPI triad remain required before synthesis closeout.",
        }
    )
    ...
    row.update(
        {
            ...
            "objective": "Bounded synthesis special_mixing mix-3 proxy execution after Wave03 cadence gate.",
            "axis_tags": "bounded_synthesis;special_mixing;mix-3;tradeability_control;volatility_state_negative_memory",
            "primary_family": "model_training",
            "primary_skill": "spacesonar-model-validation",
            "claim_boundary": CLAIM_BOUNDARY,
    ...
    row.update(
        {
            ...
            "evidence_path": SUMMARY_PATH.as_posix(),
            "next_action": NEXT_WORK_ITEM_ID,
            "notes": "Mix-3 proxy execution recorded; no candidate/runtime/economics claim.",
        }
    )
    base.write_csv_rows(synthesis_path, fields, rows)


def refresh_kpi_ledger(summary: dict[str, Any]) -> None:
    ...
    kpi_summary = base.read_yaml(repo_path(KPI_SUMMARY_PATH))
    kpi_summary.update(
        {
            "updated_at_utc": summary["created_at_utc"],
            "status": "mix3_proxy_kpi_triad_refreshed_policy_bound",
    ...
    kpi_summary.update(
        writer_fields(
            writer_owned_outputs=[KPI_SUMMARY_PATH],
            source_of_truth_paths=kpi_source_paths,
            progress_effect="mix3_proxy_kpi_triad_summary_refreshed",
            experiment_or_boundary_effect="kpi_triad_records_mix3_proxy_rows_before_l4_follow_through",
            primary_family="model_training",
            primary_skill="spacesonar-model-validation",
            claim_boundary=KPI_CLAIM_BOUNDARY,
        )
    )
    base.write_machine_yaml(repo_path(KPI_SUMMARY_PATH), kpi_summary)

    kpi_manifest = base.read_yaml(repo_path(KPI_LEDGER_MANIFEST_PATH))
    kpi_manifest["updated_at_utc"] = summary["created_at_utc"]
    kpi_manifest["status"] = "mix3_proxy_kpi_ledger_manifest_policy_bound"
    kpi_manifest["claim_boundary"] = KPI_CLAIM_BOUNDARY
    kpi_manifest["score_probe_mt5_kpi_policy"] = "non_trading_score_probe_excluded_from_campaign_kpi_ledger_by_contract"
    kpi_manifest["next_action"] = NEXT_ACTION
    kpi_manifest.setdefault("summary", {})["sha256"] = base.file_sha256(repo_path(KPI_SUMMARY_PATH))
    kpi_manifest.update(
        writer_fields(
            writer_owned_outputs=[KPI_LEDGER_MANIFEST_PATH],
            source_of_truth_paths=[KPI_SUMMARY_PATH, *kpi_source_paths],
            progress_effect="mix3_proxy_kpi_ledger_manifest_refreshed",
            experiment_or_boundary_effect="kpi_manifest_records_mix3_proxy_rows_before_l4_follow_through",
            primary_family="model_training",
            primary_skill="spacesonar-model-validation",
            claim_boundary=KPI_CLAIM_BOUNDARY,
        )
    )
    base.write_machine_yaml(repo_path(KPI_LEDGER_MANIFEST_PATH), kpi_manifest)
...
def update_control_records(summary: dict[str, Any], results: list[dict[str, str]]) -> None:
    ...
    run_manifest.update(
        writer_fields(
            ...
            progress_effect="mix3_run_specs_manifest_records_proxy_execution",
            experiment_or_boundary_effect="mix3_specs_executed_without_changing_locked_matrix",
        )
    )
    ...
    for item in queue.get("mix_items", []):
        if item.get("mix_item_id") == MIX_ITEM_ID:
            ...
            item["status"] = "pending_after_mix3_l4_and_kpi_evidence"
    ...
    campaign.update(
        writer_fields(
            ...
            progress_effect="bounded_synthesis_campaign_records_mix3_proxy_execution",
            experiment_or_boundary_effect="campaign_active_pointer_moved_to_mix3_l4_follow_through",
        )
    )
    ...
    for allocation in wave.get("campaign_allocations", []):
        if allocation.get("campaign_id") == CAMPAIGN_ID:
            ...
            allocation["notes"] = "Mix-3 proxy batch executed; L4 follow-through and KPI triad required next."
    wave.update(
        writer_fields(
            writer_owned_outputs=[PATHS["wave_allocation"]],
            source_of_truth_paths=[PATHS["campaign_manifest"], SUMMARY_PATH, INDEX_PATH],
            progress_effect="wave_allocation_records_mix3_proxy_execution",
    ...
    for ref in refs:
        if ref.get("campaign_id") == CAMPAIGN_ID:
            ...
            ref["notes"] = "Mix-3 proxy batch executed; L4 follow-through and KPI triad required next."
    ...
    goal.setdefault("wave03_bounded_synthesis_special_mixing", {}).update(
        {
            "status": STATUS,
            "claim_boundary": CLAIM_BOUNDARY,
            "mix3_proxy_execution_summary": SUMMARY_PATH.as_posix(),
            "mix3_proxy_execution_index": INDEX_PATH.as_posix(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute Wave03 bounded-synthesis mix-3 proxy run specs.")

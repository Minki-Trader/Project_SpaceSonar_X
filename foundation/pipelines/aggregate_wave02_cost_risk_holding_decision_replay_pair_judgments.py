...
import foundation.pipelines.run_wave02_cost_risk_holding_l4_decision_replay_attempts as runtime_writer
...
WORK_ITEM_ID = "work_wave02_cost_risk_holding_decision_replay_judgment_v0"
NEXT_WORK_ITEM_ID = "work_wave02_cost_risk_holding_decision_replay_l5_routing_decision_v0"
...
SOURCE_PAIR_INDEX = Path(
    "lab/campaigns/campaign_us100_wave02_cost_risk_holding_surface_v0/"
...
PAIR_CLOSEOUT = Path(
    ...
    "work_wave02_cost_risk_holding_decision_replay_judgment_v0_closeout.yaml"
)
...
CLAIM_BOUNDARY = (
    "wave02_cost_risk_holding_decision_replay_pair_judgment_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_cost_risk_holding_decision_replay_l5_routing_decision_pending_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
STATUS = "wave02_cost_risk_holding_decision_replay_pair_judgment_completed_l5_routing_decision_pending"
NEXT_ACTION = (
    "decide whether a bounded candidate-specific L5 routing review is justified; "
...
def delta(left: float | None, right: float | None) -> float | str:
...
def aggregate_pairs(started_at_utc: str, command_argv: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for cell_id in sorted(rows_by_cell):
        ...
        open_rate_values = [
            value for value in [safe_float(validation_open_rate), safe_float(research_open_rate)] if value is not None
        ]
        failed_rate_values = [
    ...
    summary = {
        "version": "wave02_cost_risk_holding_decision_replay_pair_judgment_summary_v1",
        "summary_id": "wave02_cost_risk_holding_decision_replay_pair_judgment_summary_v0",
        ...
        "id_chain": {
            ...
            "sweep_id": SWEEP_ID,
            "run_id": None,
            "artifact_id": "artifact_wave02_cost_risk_holding_decision_replay_pair_judgment_summary_v0",
            "bundle_id": None,
            "candidate_id": None,
        },
        "campaign_id": CAMPAIGN_ID,
        ...
        "status": STATUS if all_pairs_complete else "wave02_cost_risk_holding_decision_replay_pair_judgment_partial_progress",
        "claim_boundary": CLAIM_BOUNDARY,
        ...
        "judgment": {
            "result_subject": "Wave02 CRH decision replay validation/research_oos action telemetry pair aggregation",
            ...
            "comparison_baseline": "source Wave02 L4 score pair judgment plus paired decision replay action observation",
            "tested_factor": "sparse score_band_side direction policy replayed from Wave02 cost/risk/holding score telemetry",
        ...
        "unresolved_blockers": ["Wave02_decision_replay_L5_routing_decision_pending"],
        "reopen_conditions": [
...
def upsert_artifact_registry(summary: dict[str, Any]) -> None:
    ...
    put(
        "artifact_wave02_cost_risk_holding_decision_replay_pair_judgment_summary_v0",
        "decision_replay_pair_judgment_summary",
        PAIR_SUMMARY,
        "Wave02 CRH decision replay validation/research_oos pair judgment summary",
    )
    put(
        "artifact_wave02_cost_risk_holding_decision_replay_pair_judgment_index_v0",
        "decision_replay_pair_judgment_index",
        PAIR_INDEX,
        "Wave02 CRH decision replay pair-level action telemetry judgment index",
    )
    put(
        "artifact_wave02_cost_risk_holding_decision_replay_pair_judgment_closeout_v0",
        "work_closeout",
        PAIR_CLOSEOUT,
        "Wave02 CRH decision replay pair judgment closeout",
    )
...
def next_work_payload(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "next_action": NEXT_ACTION,
        "status": "wave02_cost_risk_holding_decision_replay_l5_routing_decision_pending",
        "current_truth": {
...
def update_control_records(summary: dict[str, Any]) -> None:
    ...
    counts["wave02_cost_risk_holding_decision_replay_pair_judgment"] = summary["counts"]
    ...
    if (REPO_ROOT / GOAL_REGISTRY).exists():
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                row["claim_boundary"] = "active_goal_wave02_cost_risk_holding_decision_replay_l5_routing_decision_not_goal_achieve"
        if goal_rows:
...
def writer_scope_self_check(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    for artifact_id, path in [
        ("artifact_wave02_cost_risk_holding_decision_replay_pair_judgment_summary_v0", PAIR_SUMMARY),
        ("artifact_wave02_cost_risk_holding_decision_replay_pair_judgment_index_v0", PAIR_INDEX),
        ("artifact_wave02_cost_risk_holding_decision_replay_pair_judgment_closeout_v0", PAIR_CLOSEOUT),
    ]:
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/aggregate_wave02_cost_risk_holding_decision_replay_pair_judgments.py"]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate Wave02 CRH decision replay runtime pair judgments.")

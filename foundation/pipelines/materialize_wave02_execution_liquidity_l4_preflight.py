...
from foundation.features.wave02_execution_liquidity_features import build_wave02_execution_liquidity_features  # noqa: E402
from foundation.labels.wave02_execution_liquidity_labels import build_wave02_execution_liquidity_labels  # noqa: E402
from foundation.onnx.skl2onnx_adapters import convert_sklearn_pipeline_for_lab  # noqa: E402
from foundation.pipelines.run_wave02_execution_liquidity_proxy_batch import (  # noqa: E402
    build_model_target,
    load_row_membership,
    model_family,
    split_masks,
...
WORK_ITEM_ID = "work_wave02_execution_liquidity_l4_materialization_preflight_v0"
NEXT_WORK_ITEM_ID = "work_wave02_execution_liquidity_l4_runtime_execution_v0"
...
CLOSEOUT_PATH = Path("lab/goals") / GOAL_ID / "work_wave02_execution_liquidity_l4_materialization_preflight_v0_closeout.yaml"
...
COMMON_REL_ROOT = "SpaceSonar\\wave02_execution_liquidity_l4_score_probe"
EA_EXPERT_CONFIG_PATH = "Project_SpaceSonar_X\\foundation\\mt5\\experts\\SpaceSonar_ONNX_L4_ScoreProbe.ex5"

BUNDLE_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l4_onnx_bundle_preflight_python_parity_only_no_runtime_authority_"
    "no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
ATTEMPT_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l4_strategy_tester_attempt_preparation_only_no_runtime_authority_"
    "no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l4_attempts_prepared_terminal_execution_pending_no_runtime_authority_"
    "no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
ONNX_STATUS = "onnx_exported_python_onnxruntime_parity_passed"
ATTEMPT_STATUS = "prepared_pending_terminal_execution"
CAMPAIGN_STATUS = "wave02_execution_liquidity_l4_attempts_prepared_terminal_execution_next"
...
def write_crlf_text(path: Path, text: str, *, encoding: str = "utf-8") -> None:
    os.makedirs(filesystem_path(path.parent), exist_ok=True)
    with open(filesystem_path(path), "w", encoding=encoding, newline="\r\n") as handle:
        handle.write(text)
...
def cell_id_from_run_id(run_id: str) -> str:
    match = re.search(r"(wave02_elq_cell_\d{3})", run_id)
    if not match:
        raise ValueError(f"cannot derive Wave02 execution/liquidity cell_id from run_id={run_id}")
...
def normalize_proxy_summary_for_writer_smoke() -> None:
    path = REPO_ROOT / PROXY_SUMMARY
    if not path_exists(path):
        return
    summary = read_yaml(path)
    counts = summary.get("counts") or {}
    changed = False
    expected_run_count = counts.get("executed_proxy_run_count")
    expected_result_counts = counts.get("result_judgment_counts") or {}
    if summary.get("executed_proxy_run_count") != expected_run_count:
        summary["executed_proxy_run_count"] = expected_run_count
        changed = True
    if summary.get("result_counts") != expected_result_counts:
        summary["result_counts"] = expected_result_counts
        changed = True
    for key in ["runtime_authority", "economics_pass", "live_readiness"]:
        if summary.get(key) != "not_claimed":
            summary[key] = "not_claimed"
            changed = True
    if changed:
        write_yaml(path, summary)


def normalize_pre_l4_run_records(run_manifest_path: Path) -> dict[str, Any]:
    ...
    manifest["id_chain"] = {
        "goal_id": manifest.get("active_goal_id") or GOAL_ID,
        "wave_id": manifest.get("wave_id") or WAVE_ID,
        "campaign_id": manifest.get("campaign_id") or CAMPAIGN_ID,
        "idea_id": manifest.get("idea_id"),
        "hypothesis_id": manifest.get("hypothesis_id"),
        "surface_id": manifest.get("surface_id") or SURFACE_ID,
        "sweep_id": manifest.get("sweep_id") or SWEEP_ID,
        "run_id": run_id,
        "artifact_id": None,
        "bundle_id": None,
        "candidate_id": None,
    }
    storage = manifest.setdefault("storage_contract", {})
    storage.update(
        {
            "source_of_truth": repo_relative(REPO_ROOT, run_manifest_path),
            "receipt": repo_relative(REPO_ROOT, receipt_path),
            "lineage": repo_relative(REPO_ROOT, lineage_path),
            "metrics": repo_relative(REPO_ROOT, metrics_path),
    ...
    for item in [
        ...
        ensure_list_item(passed, item)
    missing = list(coverage.get("missing") or [])
    ensure_list_item(missing, "L4_split_runtime_probe_for_valid_proxy_run")
    not_applicable = list(coverage.get("not_applicable") or [])
    ensure_list_item(not_applicable, "locked_final_oos_b_access")
    coverage.update({"passed": passed, "missing": missing, "not_applicable": not_applicable})
    write_json(run_manifest_path, manifest)

    if path_exists(lineage_path):
        lineage = read_json(lineage_path)
        source_inputs = list(lineage.get("source_inputs") or [])
        manifest_ref = artifact_ref(run_manifest_path)
        replaced = False
        for index, item in enumerate(source_inputs):
            if item.get("path") == repo_relative(REPO_ROOT, run_manifest_path):
                source_inputs[index] = manifest_ref
                replaced = True
                break
        if not replaced:
            source_inputs.append(manifest_ref)
        lineage["source_inputs"] = source_inputs
...
def materialize_bundle(
    *,
    ...
    refs = run_spec["recipes"]
    axis_values = run_spec["axis_values"]
    ...
    features, feature_schema = build_wave02_execution_liquidity_features(
        frame,
        refs["feature_recipe_id"],
        str(axis_values["feature_variant"]),
    )
    labels, label_schema = build_wave02_execution_liquidity_labels(
        frame,
        refs["label_recipe_id"],
    ...
    x = features[columns]
    model_family_name = model_family(axis_values)
    fit = fit_proxy_model(
        x,
        target,
        train_mask,
    ...
    feature_schema_contract = {
        ...
        "boundary": "right_aligned_US100_M5_closed_bar_wave02_execution_liquidity_features",
    ...
    write_json(
        artifacts_dir / "model_summary.json",
        {
            ...
            "proxy_model_family": model_family_name,
            "task_kind": task_kind,
            "target_name": target_name,
            "target_threshold": target_threshold,
    ...
    bundle = {
        ...
        "target_and_label": {
            ...
            "label_variant": str(axis_values["label_variant"]),
            "target_name": target_name,
            "target_threshold": target_threshold,
        ...
        "task_surface": {
            "task_type": "execution_liquidity_tradeability_proxy",
            "target_or_label": refs["label_recipe_id"],
            "direction_mapping": "execution_liquidity_proxy_score_not_runtime_authority",
            "output_head": "single_score_for_L4_non_trading_score_probe",
        },
        "decision_use": "wave02_score_telemetry_preflight_for_execution_liquidity_surface",
        ...
        "decision_surface": {
            ...
            "execution_variant": str(axis_values["execution_variant"]),
            "holding_variant": str(axis_values["holding_variant"]),
            "execution_policy": "not_executed_score_telemetry_only",
        ...
        "model_family": model_family_name,
        "model_task": "wave02_execution_liquidity_classification",
        ...
        "parser_version": "spacesonar_wave02_execution_liquidity_l4_preflight_v0",
        ...
        "proxy_runtime_parity": {
            ...
            "known_differences": [
                ...
                "Wave02 execution/liquidity decision recipes may use label, execution, and holding variants; this L4 score probe records non-trading score telemetry only.",
            ...
            "interpretation_drift_risks": [
                ...
                "execution_liquidity_label_context_not_available_to_score_only_EA",
            ...
            "prevention_memory": [
                ...
                "Do not convert Wave02 execution/liquidity proxy metrics into economics claims from score telemetry.",
        ...
        "provenance": {
            ...
            "input_hashes": [
                ...
                artifact_ref(REPO_ROOT / Path(run_spec["storage_contract"]["source_of_truth"])),
            ],
            "output_hashes": [
    ...
    return {
        ...
        "proxy_runtime_parity": {
            ...
            "prevention_memory": [
                ...
                "Do not infer execution/liquidity economics from non-trading score telemetry.",
            ],
    ...
    *,
    ...
    for bundle in bundles:
        ...
        for period in periods:
            ...
            write_crlf_text(tester_config_path, config_text)
...
def build_summaries(
    *,
    ...
    runtime_period_set_id = runtime_contract["period_authority"]["default_runtime_period_set_id"]
    precheck = {
        "version": "writer_contract_precheck_v2",
        ...
        "broad_validation_escalation_reason": "none_l4_materialization_preflight_no_protected_claim",
        "writer_scope_self_check": "pending_after_write",
        "forbidden_claims": FORBIDDEN_CLAIMS,
        "unresolved_blockers_or_none": ["L4_split_runtime_probe_terminal_execution_pending"],
        "next_action": NEXT_WORK_ITEM_ID,
        "next_action_or_reopen_condition": NEXT_WORK_ITEM_ID,
    }
    onnx_summary = {
        "version": "wave02_execution_liquidity_l4_onnx_materialization_summary_v1",
        "writer_contract_version": WRITER_CONTRACT_VERSION,
        "summary_id": "wave02_execution_liquidity_l4_onnx_materialization_summary_v0",
        "work_item_id": WORK_ITEM_ID,
        "active_goal_id": GOAL_ID,
        "campaign_id": CAMPAIGN_ID,
        "sweep_id": SWEEP_ID,
        ...
        "primary_family": "onnx_export_parity",
        "primary_skill": "spacesonar-runtime-evidence",
        "source_of_truth_paths": [ONNX_SUMMARY.as_posix(), ONNX_INDEX.as_posix()],
        "writer_owned_outputs": [
            "runtime/packages/<bundle_id>/experiment_bundle.json",
            ONNX_SUMMARY.as_posix(),
            ONNX_INDEX.as_posix(),
        ...
        "non_pytest_smokes": ["py_compile", "writer_scope_self_check", "writer_scope_evidence_smoke"],
        "skipped_broad_validations": precheck["skipped_broad_validations"],
        "broad_validation_escalation_reason": precheck["broad_validation_escalation_reason"],
        "writer_scope_self_check": "pending_after_write",
        "unresolved_blockers_or_none": ["L4_split_runtime_probe_terminal_execution_pending"],
        "next_action_or_reopen_condition": NEXT_WORK_ITEM_ID,
        "source_inputs": {
    ...
    attempt_summary = {
        "version": "wave02_execution_liquidity_l4_attempt_preparation_summary_v1",
        "writer_contract_version": WRITER_CONTRACT_VERSION,
        "summary_id": "wave02_execution_liquidity_l4_attempt_preparation_summary_v0",
        "work_item_id": NEXT_WORK_ITEM_ID,
        "preparation_work_item_id": WORK_ITEM_ID,
        "active_goal_id": GOAL_ID,
        "campaign_id": CAMPAIGN_ID,
        ...
        "primary_family": "runtime_probe",
        "primary_skill": "spacesonar-runtime-evidence",
        "source_of_truth_paths": [ATTEMPT_SUMMARY.as_posix(), ATTEMPT_INDEX.as_posix()],
        "writer_owned_outputs": [
            "runtime/mt5_attempts/<attempt_id>/attempt_manifest.yaml",
        ...
        "validation_depth": "writer_scope_smoke",
        "non_pytest_smokes": ["py_compile", "writer_scope_self_check", "active_pointer_smoke"],
        "skipped_broad_validations": precheck["skipped_broad_validations"],
        "broad_validation_escalation_reason": precheck["broad_validation_escalation_reason"],
        "writer_scope_self_check": "pending_after_write",
        "unresolved_blockers_or_none": ["L4_split_runtime_probe_terminal_execution_pending"],
        ...
        "prevention_memory": [
            ...
            "Wave02 execution/liquidity proxy semantics require a later decision-execution adapter before economics can be discussed.",
        ],
...
def build_closeout(onnx_summary: dict[str, Any], attempt_summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "source_of_truth_paths": [PRECHECK_SUMMARY.as_posix(), ONNX_SUMMARY.as_posix(), ATTEMPT_SUMMARY.as_posix()],
        "writer_owned_outputs": [
            PRECHECK_SUMMARY.as_posix(),
            ONNX_SUMMARY.as_posix(),
            ONNX_INDEX.as_posix(),
            ATTEMPT_SUMMARY.as_posix(),
            ATTEMPT_INDEX.as_posix(),
        ...
        "validation_depth": "writer_scope_smoke",
        "non_pytest_smokes": ["py_compile", "writer_scope_self_check", "writer_scope_evidence_smoke", "active_pointer_smoke", "machine_yaml_identity_lint"],
        "skipped_broad_validations": onnx_summary["skipped_broad_validations"],
        "broad_validation_escalation_reason": onnx_summary["broad_validation_escalation_reason"],
        "writer_scope_self_check": "pending_after_write",
...
def writer_scope_self_check_payload(bundle_rows: list[dict[str, Any]], attempt_rows: list[dict[str, Any]]) -> dict[str, Any]:
    required_paths = [
        PRECHECK_SUMMARY.as_posix(),
        ONNX_SUMMARY.as_posix(),
        ONNX_INDEX.as_posix(),
    ...
    required_paths.extend(row["bundle_manifest_path"] for row in bundle_rows)
    required_paths.extend(row["attempt_manifest_path"] for row in attempt_rows)
    required_paths.extend(row["tester_config_path"] for row in attempt_rows)
    failures = [path for path in required_paths if not path_exists(REPO_ROOT / path)]
    expected_attempt_count = len(bundle_rows) * 2
    if len(attempt_rows) != expected_attempt_count:
        failures.append(f"prepared_attempt_count_mismatch_expected_{expected_attempt_count}_observed_{len(attempt_rows)}")
    return {
...
def upsert_artifact_registry(bundle_rows: list[dict[str, Any]], attempt_rows: list[dict[str, Any]]) -> None:
    ...
    producer = "foundation/pipelines/materialize_wave02_execution_liquidity_l4_preflight.py --write-control-records --copy-common-files"
    ...
    for artifact_id, artifact_type, path, claim, notes in [
        ("artifact_wave02_elq_l4_writer_precheck_v0", "writer_contract_precheck", PRECHECK_SUMMARY.as_posix(), NEXT_CLAIM_BOUNDARY, "writer contract guard record"),
        ("artifact_wave02_elq_l4_onnx_materialization_summary_v0", "l4_onnx_materialization_summary", ONNX_SUMMARY.as_posix(), BUNDLE_CLAIM_BOUNDARY, "ONNX bundle preflight summary"),
        ("artifact_wave02_elq_l4_onnx_materialization_index_v0", "l4_onnx_materialization_index", ONNX_INDEX.as_posix(), BUNDLE_CLAIM_BOUNDARY, "ONNX bundle preflight index"),
        ("artifact_wave02_elq_l4_attempt_preparation_summary_v0", "l4_attempt_preparation_summary", ATTEMPT_SUMMARY.as_posix(), ATTEMPT_CLAIM_BOUNDARY, "prepared attempt summary; terminal execution pending"),
        ("artifact_wave02_elq_l4_attempt_preparation_index_v0", "l4_attempt_preparation_index", ATTEMPT_INDEX.as_posix(), ATTEMPT_CLAIM_BOUNDARY, "prepared attempt index"),
        ("artifact_wave02_elq_l4_score_probe_ea_source_v0", "mt5_ea_source", EA_SOURCE.as_posix(), ATTEMPT_CLAIM_BOUNDARY, "non-trading score telemetry EA source"),
    ]:
        put(
            {
...
def update_control_records(onnx_summary: dict[str, Any], attempt_summary: dict[str, Any], closeout: dict[str, Any]) -> None:
    ...
    next_work = {
        ...
        "acceptance_criteria": [
            "execute prepared validation and research_oos MT5 execution/liquidity score-probe attempts",
        ...
        "broad_validation_escalation_reason": "none_runtime_execution_pending_no_protected_claim",
        "writer_scope_self_check": "required_before_close",
        "next_action": "execute_prepared_wave02_execution_liquidity_l4_strategy_tester_attempts",
        "path": NEXT_WORK_ITEM.as_posix(),
        "summary": "Execute Wave02 execution/liquidity prepared L4 Strategy Tester score-probe attempts.",
        "provenance": {
            "source": "wave02_execution_liquidity_l4_materialization_preflight",
            "campaign_id": CAMPAIGN_ID,
        ...
        "next_action_or_reopen_condition": "execute_prepared_wave02_execution_liquidity_l4_strategy_tester_attempts",
    }
    ...
    resume["cursor_state"] = "active_wave02_execution_liquidity_l4_attempts_prepared_terminal_execution_next"
    ...
    goal["next_work_item"] = {
        "work_item_id": NEXT_WORK_ITEM_ID,
        "path": NEXT_WORK_ITEM.as_posix(),
        "summary": "Execute prepared Wave02 execution/liquidity L4 score-probe attempts.",
    }
    ...
    fieldnames, goal_rows = read_csv_with_fieldnames(goal_registry_path)
    for row in goal_rows:
        if row.get("goal_id") == GOAL_ID:
            row["active_phase"] = CAMPAIGN_STATUS
            row["claim_boundary"] = NEXT_CLAIM_BOUNDARY
            row["next_work_item"] = NEXT_WORK_ITEM_ID
            break
    else:
        raise RuntimeError(f"goal registry row not found for {GOAL_ID}")
    write_csv(goal_registry_path, goal_rows, fieldnames)
    ...
    workspace["next_action"] = "execute_prepared_wave02_execution_liquidity_l4_strategy_tester_attempts"
    workspace["unresolved_blockers"] = ["L4_split_runtime_probe_terminal_execution_pending"]
    workspace["active_campaign"] = {
        "campaign_id": CAMPAIGN_ID,
        "status": CAMPAIGN_STATUS,
        "manifest": (CAMPAIGN_DIR / "campaign_manifest.yaml").as_posix(),
    }
    workspace["active_work_item"] = {"work_item_id": NEXT_WORK_ITEM_ID, "path": NEXT_WORK_ITEM.as_posix()}
    workspace["wave02_execution_liquidity_l4_materialization"] = {
        "campaign_id": CAMPAIGN_ID,
        "status": CAMPAIGN_STATUS,
        "claim_boundary": NEXT_CLAIM_BOUNDARY,
        "onnx_materialization_summary": ONNX_SUMMARY.as_posix(),
    ...
    workspace.setdefault("summary_counts", {})["wave02_execution_liquidity_l4_materialization"] = {
        "exported_bundle_count": attempt_summary["counts"]["exported_bundle_count"],
        "prepared_attempt_count": attempt_summary["counts"]["prepared_attempt_count"],
        "candidate_count": 0,
        "l5_candidate_count": 0,
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize Wave02 execution/liquidity ONNX bundles and prepared L4 MT5 attempts.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    command_argv = sys.argv[:] if argv is None else ["foundation/pipelines/materialize_wave02_execution_liquidity_l4_preflight.py", *argv]
    ...
    normalize_proxy_summary_for_writer_smoke()
    rows = run_refs()
    ...
    for row in rows:
        ...
        normalize_pre_l4_run_records(run_manifest_path)
        spec = load_run_spec(run_manifest_path)
    ...
    self_check = writer_scope_self_check_payload(bundle_rows, attempt_rows)
    precheck["writer_scope_self_check"] = self_check
    onnx_summary["writer_scope_self_check"] = self_check
    attempt_summary["writer_scope_self_check"] = self_check
    ...
    if self_check["status"] != "passed":
        raise RuntimeError(f"writer scope self-check failed: {self_check['missing_or_failed']}")

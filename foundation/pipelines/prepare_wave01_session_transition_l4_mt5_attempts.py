...
SUBWORK_ID = "work_wave01_session_transition_l4_mt5_attempt_preparation_v0"
CAMPAIGN_ID = "campaign_us100_session_transition_regime_surface_v0"
SWEEP_ID = "sweep_us100_session_transition_broad_v0"
SUMMARY_ID = "wave01_session_transition_l4_mt5_attempt_preparation_summary_v0"
CLAIM_BOUNDARY = (
    "wave01_session_transition_l4_strategy_tester_attempt_preparation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate"
)
NEXT_PHASE = "wave01_session_transition_l4_attempts_prepared_terminal_execution_next"

MATERIALIZATION_SUMMARY = Path(
    "lab/campaigns/campaign_us100_session_transition_regime_surface_v0/l4_follow_through/"
...
CLOSEOUT_PATH = Path(
    ...
    "work_wave01_session_transition_l4_mt5_attempt_preparation_v0_closeout.yaml"
)
...
def normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    summary["version"] = "wave01_session_transition_l4_mt5_attempt_preparation_summary_v1"
    ...
    summary["attempt_depth"] = "bounded_attempt_manifest_preparation"
    summary["verification_depth"] = "manifest_config_hashes_only_terminal_execution_pending"
    summary["judgment"]["next_action"] = (
        "Run the prepared Wave01 session-transition L4 Strategy Tester attempts for validation "
        "and research_oos, then close each pair with parity judgment."
    )
    summary.setdefault("prevention_memory", []).extend(
        [
            "Session-transition attempt manifests are score-probe preparation only until terminal telemetry exists.",
            "Session boundary and timeout semantics must be reconciled after MT5 telemetry before result judgment.",
        ]
    )
...
def upsert_artifact_registry(repo_root: Path, summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    producer = "foundation/pipelines/prepare_wave01_session_transition_l4_mt5_attempts.py --write-control-records --copy-common-files"
    ...
    put(
        {
            "artifact_id": "artifact_wave01_session_transition_l4_attempt_preparation_summary_v0",
            ...
            "notes": "session-transition attempt preparation only; Strategy Tester execution remains pending",
        }
    )
    put(
        {
            "artifact_id": "artifact_wave01_session_transition_l4_attempt_preparation_index_v0",
            ...
            "consumer": WORK_ITEM_ID,
            "claim_boundary": CLAIM_BOUNDARY,
            "notes": "session-transition attempt index for 10 bundles x validation/research_oos roles",
        }
    )
    put(
        {
            "artifact_id": "artifact_wave01_session_transition_l4_attempt_preparation_closeout_v0",
    ...
    put(
        {
            "artifact_id": "artifact_wave01_session_transition_l4_score_probe_ea_source_v0",
            ...
            "consumer": WORK_ITEM_ID,
            "claim_boundary": CLAIM_BOUNDARY,
            "notes": "non-trading full-period score telemetry probe reused for session-transition",
        }
    )
        ...
        put(
            {
                ...
                "consumer": WORK_ITEM_ID,
                "claim_boundary": CLAIM_BOUNDARY,
                "notes": "prepared session-transition L4 MT5 attempt; terminal execution pending",
            }
        )
        put(
            {
                ...
                "consumer": WORK_ITEM_ID,
                "claim_boundary": CLAIM_BOUNDARY,
                "notes": "session-transition MT5 Strategy Tester config for one period role",
            }
        )
...
def update_control_records(
    ...
    current_truth["wave01_session_transition_l4_attempt_preparation_summary"] = SUMMARY_PATH.as_posix()
    current_truth["wave01_session_transition_l4_attempt_preparation_status"] = summary["status"]
    current_truth["wave01_session_transition_l4_attempt_preparation_counts"] = summary["counts"]
    ...
    next_work["missing_material_if_relevant"] = [
        "L4_strategy_tester_terminal_execution_absent_for_wave01_session_transition_prepared_attempts",
    ...
    session_transition["l4_attempt_preparation_summary"] = SUMMARY_PATH.as_posix()
    session_transition["l4_attempt_preparation_status"] = summary["status"]
    session_transition["l4_attempt_preparation_counts"] = summary["counts"]
    ...
    claims["wave0_third_campaign_L4_status"] = "L4_attempts_prepared_terminal_execution_required_next"
    claims["wave0_third_campaign_l4_attempt_preparation_summary"] = SUMMARY_PATH.as_posix()
    claims["wave0_third_campaign_l4_attempt_preparation_status"] = summary["status"]
    claims["wave0_third_campaign_l4_attempt_preparation_counts"] = summary["counts"]
    ...
    if goal_registry_path.exists():
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                row["claim_boundary"] = "active_goal_wave01_session_transition_l4_attempts_prepared_not_goal_achieve"
        if goal_rows:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare Wave01 session-transition L4 MT5 Strategy Tester attempts.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    command_argv = ["python", "foundation/pipelines/prepare_wave01_session_transition_l4_mt5_attempts.py"]
    ...
    summary, rows, manifests, configs = base.build_attempt_rows_and_manifests(
        repo_root,

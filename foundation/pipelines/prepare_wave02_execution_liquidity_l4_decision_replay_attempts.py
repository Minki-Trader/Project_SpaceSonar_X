...
PARENT_WORK_ITEM_ID = "work_wave02_execution_liquidity_l5_routing_decision_v0"
WORK_ITEM_ID = "work_wave02_execution_liquidity_l4_decision_replay_adapter_preparation_v0"
NEXT_WORK_ITEM_ID = "work_wave02_execution_liquidity_decision_replay_runtime_execution_v0"
...
ADAPTER_INPUT_SUMMARY_PATH = OUTPUT_DIR / "adapter_input_summary.yaml"
...
SOURCE_PREP_INDEX = Path(
    "lab/campaigns/campaign_us100_wave02_execution_liquidity_surface_v0/l4_follow_through/l4_attempt_preparation_index.csv"
)
CLOSEOUT_PATH = Path(
    "lab/goals/goal_us100_onnx_forward_boundary_v0/"
    "work_wave02_execution_liquidity_l4_decision_replay_adapter_preparation_v0_closeout.yaml"
)
...
COMMON_DECISION_ROOT = "SpaceSonar\\wave02_execution_liquidity_l4_decision_replay"
ADAPTER_DECISION_FAMILY = "abstain_capable_direction_agnostic_tradeability"
ADAPTER_DECISION_OUTPUT = "score_ge_high_tradeable_then_momentum_ret_1_side_else_flat"

CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_decision_replay_adapter_preparation_only_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_decision_replay_runtime_execution_pending_"
    "no_runtime_authority_no_economics_pass_no_candidate_no_live_readiness_no_goal_achieve"
)
STATUS = "wave02_execution_liquidity_decision_replay_adapter_attempts_prepared_terminal_execution_next"
NEXT_STATUS = "wave02_execution_liquidity_decision_replay_terminal_execution_pending"
NEXT_ACTION = (
    "execute prepared Wave02 execution/liquidity decision replay MT5 attempts with portable terminal only, "
    "then judge paired validation/research_oos results"
)
FORBIDDEN_CLAIMS = [
    "selected_baseline",
    "operating_reference",
...
BROAD_VALIDATION_ESCALATION_REASON = "none_adapter_preparation_progress_no_protected_claim"
...
def common_relative_to_path(common_relative_path: str) -> Path | None:
    root = common_files_root()
    if root is None:
        return None
    return root / Path(common_relative_path.replace("\\", os.sep))


def local_score_telemetry_path(source_attempt: dict[str, str]) -> Path:
    attempt_manifest = Path(source_attempt["attempt_manifest_path"])
    return attempt_manifest.parent / "telemetry" / "score_telemetry.csv"
...
def label_schema_path(bundle_id: str) -> Path:
    return Path("runtime/packages") / bundle_id / "artifacts" / "label_schema.json"


def load_label_schema(bundle_id: str) -> dict[str, Any]:
    return read_json(REPO_ROOT / label_schema_path(bundle_id))


def infer_hold_bars(bundle: dict[str, Any]) -> tuple[int, str]:
    explicit = (bundle.get("target_and_label") or {}).get("horizon_bars")
    if explicit is not None:
        return max(1, int(explicit)), "bundle_target_and_label.horizon_bars"
    label_schema = load_label_schema(bundle["bundle_id"])
    explicit = label_schema.get("horizon_bars")
    if explicit is not None:
        return max(1, int(explicit)), "label_schema.horizon_bars"
    label = str(bundle.get("label_recipe_id") or (bundle.get("target_and_label") or {}).get("label_surface") or "")
    match = re.search(r"_h(\d+)_", label)
    if match:
        return max(1, int(match.group(1))), "label_recipe_id_horizon_suffix"
    return 6, "fallback_h6_recorded_for_tradeability_labels_without_explicit_horizon"


def build_adapter_input_text(source_path: Path, score_high_threshold: float) -> tuple[str, dict[str, Any]]:
    full = REPO_ROOT / source_path if not source_path.is_absolute() else source_path
    with open(filesystem_path(full), "r", newline="", encoding="utf-8-sig") as handle:
        reader = csv.DictReader(handle)
        if not reader.fieldnames:
            raise ValueError(f"source score telemetry has no header: {source_path.as_posix()}")
        fieldnames = list(reader.fieldnames)
        required = {"bar_close_time", "score", "decision"}
        missing = sorted(required.difference(fieldnames))
        if missing:
            raise ValueError(f"source score telemetry missing columns {missing}: {source_path.as_posix()}")
        rows: list[dict[str, Any]] = []
        source_decisions: Counter[str] = Counter()
        adapter_decisions: Counter[str] = Counter()
        finite_scores = 0
        for source_row in reader:
            decision = str(source_row.get("decision") or "").strip().lower() or "blank"
            source_decisions[decision] += 1
            score = float(source_row["score"])
            finite_scores += 1
            adapter_decision = "tradeable" if score >= score_high_threshold else "flat"
            adapter_decisions[adapter_decision] += 1
            row = dict(source_row)
            row["decision_family"] = ADAPTER_DECISION_FAMILY
            row["decision"] = adapter_decision
            rows.append(row)
    text = dump_csv(fieldnames, rows)
    encoded = text.encode("utf-8")
    stats = {
        "source_score_telemetry_path": repo_relative(REPO_ROOT, full),
        "source_score_telemetry_sha256": sha256_file(full),
        "source_score_telemetry_size_bytes": os.stat(filesystem_path(full)).st_size,
        "source_row_count": len(rows),
        "adapter_row_count": len(rows),
        "finite_score_count": finite_scores,
        "source_decision_counts": dict(sorted(source_decisions.items())),
        "adapter_decision_counts": dict(sorted(adapter_decisions.items())),
        "adapter_tradeable_count": adapter_decisions.get("tradeable", 0),
        "adapter_flat_count": adapter_decisions.get("flat", 0),
        "adapter_input_sha256": hashlib.sha256(encoded).hexdigest(),
        "adapter_input_size_bytes": len(encoded),
        "projection_rule": ADAPTER_DECISION_OUTPUT,
    }
    return text, stats
...
def build_tester_config_text(
    *,
    ...
    lines = [
        "; SpaceSonar Wave02 execution/liquidity tradeability L4 decision replay probe.",
...
def attempt_index_fieldnames() -> list[str]:
    return [
        ...
        "source_score_telemetry_local_path",
        "source_score_telemetry_sha256",
        "source_score_telemetry_size_bytes",
        "adapter_score_telemetry_common_path",
        "adapter_score_telemetry_local_path",
        "adapter_score_telemetry_sha256",
        "adapter_score_telemetry_size_bytes",
        "adapter_common_copy_status",
        "execution_telemetry_common_path",
        "trade_shape_telemetry_common_path",
        "source_decision_family",
        "adapter_decision_family",
        "decision_family",
        "decision_execution_kind",
        "decision_output",
        "source_decision_counts_json",
        "adapter_decision_counts_json",
        "adapter_tradeable_count",
        "adapter_flat_count",
        "score_low_threshold",
    ...
    *,
    ...
    return {
        ...
        "runtime_learning_probe_decision": {
            ...
            "reason": (
                "Wave02 execution/liquidity preserved L4 tradeability score-observation pairs require a sparse decision replay "
        ...
        "runtime_surface_contract": {
            ...
            "surface_scope": "wave02_execution_liquidity_sparse_decision_replay_from_projected_mt5_score_telemetry",
            "source_score_telemetry_common_path": row["source_score_telemetry_common_path"],
            "source_score_telemetry_local_path": row["source_score_telemetry_local_path"],
            "adapter_score_telemetry_common_path": row["adapter_score_telemetry_common_path"],
            "adapter_score_telemetry_local_path": row["adapter_score_telemetry_local_path"],
            "source_score_attempt_manifest": source_attempt["attempt_manifest_path"],
            "source_decision_family": row["source_decision_family"],
            "adapter_decision_family": row["adapter_decision_family"],
            "decision_family": row["adapter_decision_family"],
            ...
            "source_decision_counts": json.loads(row["source_decision_counts_json"]),
            "adapter_decision_counts": json.loads(row["adapter_decision_counts_json"]),
            "adapter_input_projection": {
                "projection_rule": ADAPTER_DECISION_OUTPUT,
                "reason": "ELQ score-probe telemetry emitted unknown decisions for source-specific decision families; replay input projects score>=high to tradeable and otherwise flat.",
                "adapter_tradeable_count": int(row["adapter_tradeable_count"]),
                "adapter_flat_count": int(row["adapter_flat_count"]),
            },
        ...
        "proxy_runtime_parity": {
            ...
            "shared_contract": [
                ...
                "adapter_projected_tradeability_score_telemetry_full_period",
                "tradeability_score_high_threshold_from_bundle",
                "momentum_ret_1_direction_policy_from_tester_bars",
            ],
            "known_differences": [
                "This adapter replays previously observed MT5 score telemetry and does not run ONNX in the trading EA.",
                "Wave02 execution/liquidity tradeability score is direction-agnostic; direction is supplied by a declared momentum_ret_1 policy.",
                "ELQ source score telemetry decisions were unknown, so adapter input explicitly projects score>=high to tradeable and lower scores to flat.",
            ...
            "interpretation_drift_risks": [
                ...
                "source_unknown_decision_projection",
                "momentum_ret_1_direction_policy",
                "hold_bars_exit_semantics",
                "tester_fill_cost_spread_lot_rounding",
            ],
            "minimum_reconciliation_attempt": {
                "status": "prepared",
                "attempt": "score telemetry bar_close_time will be replayed into sparse MT5 orders using momentum_ret_1 side policy",
            ...
            "unit_semantics": {
                ...
                "adapter_input": "score_ge_high_tradeable_else_flat_then_momentum_ret_1_direction_policy_inside_EA",
                "lot": "fixed_lot_0.02",
            ...
            "prevention_memory": [
                "Do not infer a side/abstain candidate from score telemetry alone.",
                "Direction-agnostic tradeability requires an explicit side policy before sparse order replay.",
        ...
        "artifact_identity": {
            ...
            "source_score_telemetry": {
                "path": row["source_score_telemetry_local_path"],
                "sha256": row["source_score_telemetry_sha256"],
                "size_bytes": int(row["source_score_telemetry_size_bytes"]),
                "availability": "local_telemetry_hash_recorded_ignored_by_git",
            },
            "adapter_score_telemetry_input": {
                "path": row["adapter_score_telemetry_local_path"],
                "sha256": row["adapter_score_telemetry_sha256"],
                "size_bytes": int(row["adapter_score_telemetry_size_bytes"]),
                "common_relative_path": row["adapter_score_telemetry_common_path"],
                "common_copy_status": "pending_write",
                "availability": "local_telemetry_hash_recorded_ignored_by_git",
            },
        ...
        "required_gate_coverage": {
            "passed": [
                ...
                "adapter_score_telemetry_projected_from_score_threshold",
                "wave02_execution_liquidity_decision_adapter_prepared",
...
def build_records(
    created_at_utc: str, direction_policy: str
) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], dict[str, dict[str, Any]], dict[str, str], dict[str, str]]:
    routing_rows = [
        row
    ...
    adapter_inputs: dict[str, str] = {}
    adapter_input_records: list[dict[str, Any]] = []
    ...
    for route in sorted(routing_rows, key=lambda item: item["cell_id"]):
        ...
        label_schema = load_label_schema(pair["bundle_id"])
        source_decision_family = pair["decision_family"]
        adapter_decision_family = ADAPTER_DECISION_FAMILY
        execution_kind = decision_family_execution_kind(adapter_decision_family)
        label_family = str(label_schema.get("label_family") or "")
        eligible = (
            route.get("routing_decision") == "prepare_decision_execution_adapter"
            and pair.get("proxy_judgment") == "preserved_clue"
            and label_family.endswith("tradeability")
            and is_direct_trade_adapter_eligible(adapter_decision_family)
        )
        eligibility_reason = (
            "eligible_elq_tradeability_score_threshold_projection_momentum_ret_1_adapter"
            if eligible
            else "requires_new_decision_surface_or_non_tradeability_label"
        )
        prepared_for_cell = 0
        if eligible:
            ...
            for period_role, source_attempt_id in [
                ...
                source_local = local_score_telemetry_path(source_attempt)
                adapter_local = attempt_dir / "telemetry" / "score_replay_input.csv"
                adapter_common = f"{COMMON_DECISION_ROOT}\\{attempt_id}\\score_replay_input.csv"
                adapter_text, adapter_stats = build_adapter_input_text(source_local, high)
                adapter_inputs[attempt_id] = adapter_text
                row: dict[str, Any] = {
                    ...
                    "source_score_telemetry_local_path": adapter_stats["source_score_telemetry_path"],
                    "source_score_telemetry_sha256": adapter_stats["source_score_telemetry_sha256"],
                    "source_score_telemetry_size_bytes": adapter_stats["source_score_telemetry_size_bytes"],
                    "adapter_score_telemetry_common_path": adapter_common,
                    "adapter_score_telemetry_local_path": adapter_local.as_posix(),
                    "adapter_score_telemetry_sha256": adapter_stats["adapter_input_sha256"],
                    "adapter_score_telemetry_size_bytes": adapter_stats["adapter_input_size_bytes"],
                    "adapter_common_copy_status": "pending_write",
                    ...
                    "source_decision_family": source_decision_family,
                    "adapter_decision_family": adapter_decision_family,
                    "decision_family": adapter_decision_family,
                    "decision_execution_kind": execution_kind,
                    "decision_output": ADAPTER_DECISION_OUTPUT,
                    "source_decision_counts_json": json.dumps(adapter_stats["source_decision_counts"], sort_keys=True),
                    "adapter_decision_counts_json": json.dumps(adapter_stats["adapter_decision_counts"], sort_keys=True),
                    "adapter_tradeable_count": adapter_stats["adapter_tradeable_count"],
                    "adapter_flat_count": adapter_stats["adapter_flat_count"],
                ...
                adapter_input_records.append(
                    {
                        ...
                        "adapter_decision_family": adapter_decision_family,
                        "source_score_telemetry_local_path": row["source_score_telemetry_local_path"],
                        "source_score_telemetry_sha256": row["source_score_telemetry_sha256"],
                        "source_row_count": adapter_stats["source_row_count"],
                        "source_decision_counts": adapter_stats["source_decision_counts"],
                        "adapter_score_telemetry_local_path": row["adapter_score_telemetry_local_path"],
                        "adapter_score_telemetry_common_path": adapter_common,
                        "adapter_score_telemetry_sha256": row["adapter_score_telemetry_sha256"],
                        "adapter_input_size_bytes": row["adapter_score_telemetry_size_bytes"],
                        "adapter_row_count": adapter_stats["adapter_row_count"],
                        "adapter_tradeable_count": adapter_stats["adapter_tradeable_count"],
                        "adapter_flat_count": adapter_stats["adapter_flat_count"],
                        "adapter_decision_counts": adapter_stats["adapter_decision_counts"],
                        "projection_rule": ADAPTER_DECISION_OUTPUT,
                    }
                )
                ...
                configs[attempt_id] = build_tester_config_text(
                    attempt_id=attempt_id,
                    source_score_telemetry_common_path=row["adapter_score_telemetry_common_path"],
                    period=row,
        ...
        eligibility_rows.append(
            {
                ...
                "adapter_decision_family": adapter_decision_family,
                "decision_execution_kind": execution_kind,
                "label_variant": str(label_schema.get("label_variant") or ""),
    ...
    counts = {
        ...
        "adapter_input_total_rows": sum(int(row["adapter_tradeable_count"]) + int(row["adapter_flat_count"]) for row in attempt_rows),
        "adapter_input_tradeable_rows": sum(int(row["adapter_tradeable_count"]) for row in attempt_rows),
        "adapter_input_flat_rows": sum(int(row["adapter_flat_count"]) for row in attempt_rows),
        "period_role_counts": dict(sorted(Counter(row["period_role"] for row in attempt_rows).items())),
        "source_decision_family_counts": dict(sorted(Counter(row["source_decision_family"] for row in attempt_rows).items())),
        "adapter_decision_family_counts": dict(sorted(Counter(row["adapter_decision_family"] for row in attempt_rows).items())),
    ...
    summary = {
        "version": "wave02_execution_liquidity_l4_decision_replay_adapter_preparation_summary_v1",
        "summary_id": "wave02_execution_liquidity_l4_decision_replay_adapter_preparation_v0",
        "work_item_id": WORK_ITEM_ID,
        "parent_work_item_id": PARENT_WORK_ITEM_ID,
        ...
        "id_chain": {
            ...
            "sweep_id": SWEEP_ID,
            "run_id": None,
            "artifact_id": "artifact_wave02_execution_liquidity_decision_replay_adapter_prep_summary_v0",
            "bundle_id": None,
            "candidate_id": None,
        },
        "campaign_id": CAMPAIGN_ID,
        ...
        "created_at_utc": created_at_utc,
        "status": STATUS,
        "execution_weight": "bounded_all_preserved_wave02_pairs",
        "adapter_id": ADAPTER_ID,
        "writer_contract_version": WRITER_CONTRACT_VERSION,
        "claim_boundary": CLAIM_BOUNDARY,
        "primary_family": PRIMARY_FAMILY,
        "primary_skill": PRIMARY_SKILL,
        "support_skills": ["spacesonar-experiment-design", "spacesonar-evidence-provenance", "spacesonar-claim-discipline"],
        ...
        "source_of_truth_paths": [
            ...
            PAIR_INDEX.as_posix(),
            SOURCE_PREP_INDEX.as_posix(),
        ],
        "writer_owned_outputs": [
            SUMMARY_PATH.as_posix(),
            INDEX_PATH.as_posix(),
            ELIGIBILITY_INDEX_PATH.as_posix(),
            ADAPTER_INPUT_SUMMARY_PATH.as_posix(),
            ...
            "runtime/mt5_attempts/<attempt_id>/telemetry/score_replay_input.csv",
        ],
        "counts": counts,
        "adapter_input_projection": {
            "adapter_decision_family": ADAPTER_DECISION_FAMILY,
            "direction_policy": direction_policy,
            "projection_rule": ADAPTER_DECISION_OUTPUT,
            "source_gap": "ELQ score-probe emitted unknown source decisions for source-specific decision families",
            "repair": "project source score telemetry to tradeable/flat adapter input before Strategy Tester decision replay",
            "adapter_input_records": adapter_input_records,
        },
        "source_records": {
        ...
        "artifact_paths": {
            ...
            "adapter_input_summary": ADAPTER_INPUT_SUMMARY_PATH.as_posix(),
        ...
        "experiment_design": {
            ...
            "surface_id": SURFACE_ID,
            "sweep_id": SWEEP_ID,
            "hypothesis": "Wave02 execution/liquidity tradeability preserved clues may only matter if score-high bars can be translated into sparse side-bearing tester trades.",
            "decision_use": "score_high_tradeability_gate_with_momentum_ret_1_side_policy",
            "comparison_baseline": "Wave02 ELQ L4 non-trading score probe pair observations",
            "control_variables": [
                "FPMarkets US100 M5",
            ...
            "changed_variables": ["decision_execution_adapter", "momentum_ret_1_side_policy", "sparse_order_replay"],
            ...
            "expected_effect_probe": "preserved cells should avoid broad two-period loss before any L5 candidate-specific evidence is opened",
            "surface_rotation_rationale": "score probes are insufficient; a sparse decision replay is the smallest runtime follow-through",
            ...
            "reopen_or_stop_condition": "rerun preparation only if ELQ routing, pair judgment, source score attempt paths, or source telemetry hashes change",
            "evidence_plan": [SUMMARY_PATH.as_posix(), INDEX_PATH.as_posix(), ELIGIBILITY_INDEX_PATH.as_posix(), ADAPTER_INPUT_SUMMARY_PATH.as_posix()],
            "claim_boundary": CLAIM_BOUNDARY,
            "legacy_relation": "uses generic score replay adapter, not legacy winner or promotion inheritance",
            "axis_tags": ["decision_surface", "evaluation_or_runtime_surface", "holding_policy"],
            "broad_sweep": "bounded_all_preserved_wave02_pairs",
            "extreme_sweep": "not_applicable",
            "micro_search_gate": "forbidden_before_decision_replay_judgment",
            "failure_memory": "record negative memory if replay fails rather than repairing into candidate track",
        },
        "judgment": {
            "result_subject": "Wave02 execution/liquidity preserved-clue tradeability decision replay adapter preparation",
            "judgment_label": "runtime_probe",
            "metric_identity": "no performance metric; adapter/config preparation only",
            "comparison_baseline": "Wave02 paired L4 score telemetry preserved clues",
            "tested_factor": "direction-agnostic tradeability score translated through momentum_ret_1 sparse order policy",
            "kpi_interpretation": "not evaluated until MT5 decision replay execution",
            "directional_effect_hypothesis": "score-high tradeability bars may need explicit side policy to become testable",
        ...
        "environment": {
            "command_argv": ["python", "foundation/pipelines/prepare_wave02_execution_liquidity_l4_decision_replay_attempts.py"],
            "cwd": ".",
        ...
        "forbidden_claims": FORBIDDEN_CLAIMS,
        "unresolved_blockers_or_none": ["Wave02_execution_liquidity_decision_replay_terminal_execution_pending"],
        "next_action_or_reopen_condition": NEXT_ACTION,
    }
    smoke_errors: list[str] = []
    if len(routing_rows) != 6:
        smoke_errors.append(f"expected 6 ELQ routing rows, got {len(routing_rows)}")
    if len(attempt_rows) != 12:
        smoke_errors.append(f"expected 12 ELQ adapter attempts, got {len(attempt_rows)}")
    if counts["candidate_count"] != 0 or counts["l5_candidate_count"] != 0:
        smoke_errors.append("candidate counts must remain zero")
    if counts["adapter_input_tradeable_rows"] <= 0:
        smoke_errors.append("adapter input projection produced no tradeable rows")
    if counts["adapter_input_flat_rows"] <= 0:
        smoke_errors.append("adapter input projection produced no flat rows")
    for record in adapter_input_records:
        source_counts = record["source_decision_counts"]
        if source_counts.get("unknown", 0) != int(record["source_row_count"]):
            smoke_errors.append(f"{record['attempt_id']}: source decision count is not fully unknown; projection rule needs review")
        if int(record["adapter_tradeable_count"]) <= 0:
            smoke_errors.append(f"{record['attempt_id']}: adapter projection produced no tradeable rows")
    summary["writer_scope_self_check"] = {
        "status": "passed" if not smoke_errors else "failed",
        ...
        "routing_cell_count": len(routing_rows),
        "prepared_attempt_count": len(attempt_rows),
        "adapter_input_tradeable_rows": counts["adapter_input_tradeable_rows"],
        "adapter_input_flat_rows": counts["adapter_input_flat_rows"],
    ...
    summary["adapter_input_summary"] = build_adapter_input_summary(created_at_utc, adapter_input_records)
    summary["adapter_input_summary"]["writer_scope_self_check"] = summary["writer_scope_self_check"]
    return summary, attempt_rows, eligibility_rows, manifests, configs, adapter_inputs


def build_adapter_input_summary(created_at_utc: str, records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "version": "wave02_execution_liquidity_decision_replay_adapter_input_summary_v1",
        "summary_id": "wave02_execution_liquidity_decision_replay_adapter_input_summary_v0",
        "work_item_id": WORK_ITEM_ID,
        "parent_work_item_id": PARENT_WORK_ITEM_ID,
        "active_goal_id": GOAL_ID,
        "campaign_id": CAMPAIGN_ID,
        ...
        "created_at_utc": created_at_utc,
        "writer_contract_version": WRITER_CONTRACT_VERSION,
        "status": "adapter_inputs_projected_pending_terminal_execution",
        "adapter_id": ADAPTER_ID,
        "adapter_decision_family": ADAPTER_DECISION_FAMILY,
        "projection_rule": ADAPTER_DECISION_OUTPUT,
        "projection_reason": "ELQ score-probe source decisions were unknown; replay inputs are projected from score threshold to tradeable/flat.",
        "counts": {
            "attempt_input_count": len(records),
            "source_total_rows": sum(int(row["source_row_count"]) for row in records),
            "adapter_total_rows": sum(int(row["adapter_row_count"]) for row in records),
            "adapter_tradeable_rows": sum(int(row["adapter_tradeable_count"]) for row in records),
            "adapter_flat_rows": sum(int(row["adapter_flat_count"]) for row in records),
        },
        "records": records,
        "source_of_truth_paths": [ROUTING_INDEX.as_posix(), PAIR_INDEX.as_posix(), SOURCE_PREP_INDEX.as_posix()],
        "writer_owned_outputs": [ADAPTER_INPUT_SUMMARY_PATH.as_posix(), "runtime/mt5_attempts/<attempt_id>/telemetry/score_replay_input.csv"],
        "validation_depth": VALIDATION_DEPTH,
        "non_pytest_smokes": NON_PYTEST_SMOKES,
        "skipped_broad_validations": SKIPPED_BROAD_VALIDATIONS,
        "broad_validation_escalation_reason": BROAD_VALIDATION_ESCALATION_REASON,
...
def build_compile_summary(created_at_utc: str) -> dict[str, Any]:
    return {
        "version": "wave02_execution_liquidity_decision_replay_adapter_compile_summary_v1",
...
def build_closeout(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "unresolved_blockers_or_none": summary["unresolved_blockers_or_none"],
...
def write_records(
    ...
    adapter_records_by_attempt = {
        row["attempt_id"]: row for row in summary.get("adapter_input_summary", {}).get("records", [])
    }
    for row in attempt_rows:
        attempt_dir = REPO_ROOT / "runtime" / "mt5_attempts" / row["attempt_id"]
        attempt_dir.mkdir(parents=True, exist_ok=True)
        adapter_input_text = adapter_inputs[row["attempt_id"]]
        adapter_local_path = Path(row["adapter_score_telemetry_local_path"])
        write_text(adapter_local_path, adapter_input_text)
        common_target = common_relative_to_path(row["adapter_score_telemetry_common_path"])
        if common_target is None:
            row["adapter_common_copy_status"] = "copy_blocked_common_files_root_unavailable"
        else:
            write_text(common_target, adapter_input_text)
            row["adapter_common_copy_status"] = "copied_to_mt5_common_files"
        if row["attempt_id"] in adapter_records_by_attempt:
            adapter_records_by_attempt[row["attempt_id"]]["adapter_common_copy_status"] = row["adapter_common_copy_status"]
        ...
        manifests[row["attempt_id"]]["artifact_identity"]["adapter_score_telemetry_input"] = {
            **artifact_ref(adapter_local_path, availability="local_telemetry_hash_recorded_ignored_by_git"),
            "common_relative_path": row["adapter_score_telemetry_common_path"],
            "common_copy_status": row["adapter_common_copy_status"],
            "redacted_common_absolute_path": redact_path(str(common_target)) if common_target is not None else None,
        }
        manifests[row["attempt_id"]]["runtime_surface_contract"]["adapter_common_copy_status"] = row["adapter_common_copy_status"]
    ...
    write_yaml(ADAPTER_INPUT_SUMMARY_PATH, summary["adapter_input_summary"])
...
def upsert_artifact_registry(summary: dict[str, Any], attempt_rows: list[dict[str, Any]]) -> None:
    ...
    for artifact_id, artifact_type, path, notes in [
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_prep_summary_v0", "decision_replay_adapter_summary", SUMMARY_PATH, "summary for Wave02 decision replay adapter preparation"),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_prep_index_v0", "decision_replay_adapter_index", INDEX_PATH, "index of prepared Wave02 decision replay attempts"),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_eligibility_index_v0", "decision_replay_adapter_eligibility_index", ELIGIBILITY_INDEX_PATH, "eligibility index for Wave02 execution/liquidity preserved clue decision replay"),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_input_summary_v0", "decision_replay_adapter_input_summary", ADAPTER_INPUT_SUMMARY_PATH, "source score telemetry to adapter input projection summary for Wave02 ELQ decision replay"),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_compile_summary_v0", "ea_compile_summary", COMPILE_SUMMARY_PATH, "EA binary/source preflight for decision replay adapter"),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_prep_closeout_v0", "work_closeout", CLOSEOUT_PATH, "closeout for Wave02 decision replay adapter preparation"),
    ]:
        put(
            {
    ...
    for row in attempt_rows:
        for suffix, artifact_type, path_key in [
            ...
            ("adapter_score_replay_input_csv", "adapter_score_replay_input_csv", "adapter_score_telemetry_local_path"),
        ]:
            availability = (
                "local_telemetry_hash_recorded_ignored_by_git"
                if suffix == "adapter_score_replay_input_csv"
                else "present_hash_recorded"
            )
            put(
                {
                    ...
                    "consumer": NEXT_WORK_ITEM_ID,
                    "claim_boundary": summary["claim_boundary"],
                    "notes": "Wave02 decision replay adapter preparation; not candidate, runtime authority, or economics pass",
                }
            )
...
def update_control_records(summary: dict[str, Any]) -> None:
    ...
    output_hashes = [
        artifact_ref(path)
        for path in [SUMMARY_PATH, INDEX_PATH, ELIGIBILITY_INDEX_PATH, ADAPTER_INPUT_SUMMARY_PATH, COMPILE_SUMMARY_PATH, CLOSEOUT_PATH]
    ]
    input_hashes = [
        artifact_ref(path)
        for path in [ROUTING_SUMMARY, ROUTING_INDEX, PAIR_INDEX, SOURCE_PREP_INDEX, EXECUTION_PROFILE, RUNTIME_CONTRACT, PERIOD_PROFILE]
    ]
    next_work = {
        ...
        "acceptance_criteria": [
            "execute prepared Wave02 decision replay validation/research_oos MT5 attempts",
        ...
        "next_action": NEXT_ACTION,
        "source_of_truth_paths": [SUMMARY_PATH.as_posix(), INDEX_PATH.as_posix(), ADAPTER_INPUT_SUMMARY_PATH.as_posix()],
        "writer_owned_outputs": [
            "runtime/mt5_attempts/<attempt_id>/attempt_manifest.yaml",
            "runtime/mt5_attempts/<attempt_id>/tester_config.ini",
            "runtime/mt5_attempts/<attempt_id>/telemetry/execution_telemetry.csv",
        ...
        "summary": "Execute prepared Wave02 ELQ sparse decision replay attempts.",
        "provenance": {
            "source": WORK_ITEM_ID,
            "campaign_id": CAMPAIGN_ID,
            "source_of_truth": SUMMARY_PATH.as_posix(),
        ...
        "current_truth": {
            ...
            "decision_replay_adapter_input_summary": ADAPTER_INPUT_SUMMARY_PATH.as_posix(),
            "decision_replay_adapter_preparation_status": summary["status"],
            "decision_replay_adapter_preparation_counts": summary["counts"],
            "candidate_count": 0,
            "l5_candidate_count": 0,
        },
        "unresolved_blockers": ["Wave02_execution_liquidity_decision_replay_terminal_execution_pending"],
        "reopen_conditions": [
            "rerun adapter preparation if Wave02 ELQ routing decision, pair judgment, source score attempt index, or source telemetry hash changes",
        ...
        "writer_scope_self_check": summary["writer_scope_self_check"],
        "forbidden_claims": FORBIDDEN_CLAIMS,
        "unresolved_blockers_or_none": ["Wave02_execution_liquidity_decision_replay_terminal_execution_pending"],
        "next_action_or_reopen_condition": NEXT_ACTION,
    ...
    resume["unresolved_blockers"] = ["Wave02_execution_liquidity_decision_replay_terminal_execution_pending"]
    ...
    goal["next_work_item"] = {
        "work_item_id": NEXT_WORK_ITEM_ID,
        "path": NEXT_WORK_ITEM.as_posix(),
        "summary": "Wave02 ELQ decision replay terminal execution pending; no candidate claim.",
    }
    ...
    wave02["decision_replay_adapter_input_summary"] = ADAPTER_INPUT_SUMMARY_PATH.as_posix()
    ...
    replay["adapter_input_summary"] = ADAPTER_INPUT_SUMMARY_PATH.as_posix()
    ...
    workspace["unresolved_blockers"] = ["Wave02_execution_liquidity_decision_replay_terminal_execution_pending"]
    ...
    counts["wave02_execution_liquidity_decision_replay_adapter_preparation"] = summary["counts"]
    ...
    elq["decision_replay_adapter_preparation_summary"] = SUMMARY_PATH.as_posix()
    elq["decision_replay_adapter_preparation_index"] = INDEX_PATH.as_posix()
    elq["decision_replay_adapter_eligibility_index"] = ELIGIBILITY_INDEX_PATH.as_posix()
    elq["decision_replay_adapter_input_summary"] = ADAPTER_INPUT_SUMMARY_PATH.as_posix()
    elq["decision_replay_adapter_preparation_status"] = summary["status"]
    elq["decision_replay_adapter_preparation_counts"] = summary["counts"]
...
def smoke_outputs(summary: dict[str, Any], attempt_rows: list[dict[str, Any]]) -> list[str]:
    ...
    adapter_input_summary = read_yaml(REPO_ROOT / ADAPTER_INPUT_SUMMARY_PATH) if path_exists(REPO_ROOT / ADAPTER_INPUT_SUMMARY_PATH) else {}
    ...
    if loaded_summary.get("writer_contract_version") != WRITER_CONTRACT_VERSION:
        errors.append("summary writer_contract_version mismatch")
    if loaded_summary.get("validation_depth") != VALIDATION_DEPTH:
        errors.append("summary validation_depth mismatch")
    if (loaded_summary.get("writer_scope_self_check") or {}).get("status") != "passed":
        errors.append("writer_scope_self_check not passed")
    ...
    if counts.get("routing_cell_count") != 6:
        errors.append("expected 6 routed ELQ cells")
    ...
    if (adapter_input_summary.get("counts") or {}).get("attempt_input_count") != len(index_rows):
        errors.append("adapter input summary count mismatch")
    for row in index_rows:
        ...
        if row.get("adapter_common_copy_status") != "copied_to_mt5_common_files":
            errors.append(f"{row.get('attempt_id')}: adapter common copy status not copied")
        if row.get("adapter_decision_family") != ADAPTER_DECISION_FAMILY:
            errors.append(f"{row.get('attempt_id')}: adapter decision family mismatch")
        if int(row.get("adapter_tradeable_count") or 0) <= 0:
            errors.append(f"{row.get('attempt_id')}: adapter projection has no tradeable rows")
        for path_key in ["attempt_manifest_path", "tester_config_path", "adapter_score_telemetry_local_path"]:
            if not path_exists(REPO_ROOT / row[path_key]):
                errors.append(f"{row.get('attempt_id')}: missing {path_key}")
        adapter_path = REPO_ROOT / row["adapter_score_telemetry_local_path"]
        if path_exists(adapter_path) and row.get("adapter_score_telemetry_sha256") != sha256_file(adapter_path):
            errors.append(f"{row.get('attempt_id')}: adapter telemetry hash mismatch")
        tester_text = (REPO_ROOT / row["tester_config_path"]).read_text(encoding="utf-8-sig") if path_exists(REPO_ROOT / row["tester_config_path"]) else ""
        if f"InpScoreTelemetryPath={row['adapter_score_telemetry_common_path']}" not in tester_text:
            errors.append(f"{row.get('attempt_id')}: tester config does not point to adapter input")
        if f"InpDecisionFamily={ADAPTER_DECISION_FAMILY}" not in tester_text:
            errors.append(f"{row.get('attempt_id')}: tester config decision family mismatch")

    registry_rows = read_csv_rows(REPO_ROOT / ARTIFACT_REGISTRY)
    by_id = {row.get("artifact_id"): row for row in registry_rows}
    for artifact_id, path in [
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_prep_summary_v0", SUMMARY_PATH),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_prep_index_v0", INDEX_PATH),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_eligibility_index_v0", ELIGIBILITY_INDEX_PATH),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_input_summary_v0", ADAPTER_INPUT_SUMMARY_PATH),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_compile_summary_v0", COMPILE_SUMMARY_PATH),
        ("artifact_wave02_execution_liquidity_decision_replay_adapter_prep_closeout_v0", CLOSEOUT_PATH),
    ]:
    ...
    for attempt_row in index_rows:
        artifact_id = f"artifact_{attempt_row['attempt_id']}_adapter_score_replay_input_csv_v0"
        registry_row = by_id.get(artifact_id)
        if not registry_row:
            errors.append(f"artifact registry missing {artifact_id}")
            continue
        full = REPO_ROOT / attempt_row["adapter_score_telemetry_local_path"]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare Wave02 execution/liquidity tradeability score replay decision MT5 attempts.")
    parser.add_argument("--repo-root", default=str(REPO_ROOT))
    parser.add_argument("--direction-policy", default="momentum_ret_1", choices=SUPPORTED_DIRECTION_POLICIES)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.smoke_only:
        ...
        summary, attempt_rows, eligibility_rows, manifests, configs, adapter_inputs = build_records(created_at_utc, args.direction_policy)
        if args.write_records:
            if (summary.get("writer_scope_self_check") or {}).get("status") != "passed":
                errors = list((summary.get("writer_scope_self_check") or {}).get("smoke_errors") or ["writer_scope_self_check failed"])
            else:
                write_records(summary, attempt_rows, eligibility_rows, manifests, configs, adapter_inputs)
                update_control_records(summary)
                errors = smoke_outputs(summary, attempt_rows)
        else:
            errors = list((summary.get("writer_scope_self_check") or {}).get("smoke_errors") or [])
    ...
    if args.write_records or args.smoke_only:
        print(
            "wave02 decision replay adapter writer-smoke passed: "

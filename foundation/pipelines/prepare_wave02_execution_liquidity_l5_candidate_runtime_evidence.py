...
import foundation.pipelines.decide_wave02_execution_liquidity_decision_replay_l5_routing as routing_writer
...
WORK_ITEM_ID = "work_wave02_execution_liquidity_l5_candidate_runtime_evidence_preparation_v0"
NEXT_WORK_ITEM_ID = "work_wave02_execution_liquidity_campaign_boundary_decision_v0"
...
EVIDENCE_CLOSEOUT = Path(
    ...
    "work_wave02_execution_liquidity_l5_candidate_runtime_evidence_preparation_v0_closeout.yaml"
)
...
NON_PYTEST_SMOKES = [
    "py_compile",
    "l5_evidence_writer_smoke",
    "tester_report_kpi_parse_for_candidate_attempts",
...
BROAD_VALIDATION_ESCALATION_REASON = "none_candidate_runtime_evidence_writer_scope_only"

CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l5_candidate_runtime_evidence_observed_"
    "no_selected_baseline_no_runtime_authority_no_economics_pass_no_live_readiness_no_goal_achieve"
)
NEGATIVE_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_l5_candidate_runtime_evidence_observed_negative_"
    "no_selected_baseline_no_runtime_authority_no_economics_pass_no_live_readiness_no_goal_achieve"
)
NEXT_CLAIM_BOUNDARY = (
    "wave02_execution_liquidity_campaign_boundary_decision_pending_"
    "no_selected_baseline_no_runtime_authority_no_economics_pass_no_live_readiness_no_goal_achieve"
)
STATUS = "wave02_execution_liquidity_l5_candidate_runtime_evidence_observed_boundary_decision_pending"
NEXT_ACTION = "decide Wave02 execution/liquidity campaign boundary: close, rotate, or open bounded repair work"
...
def period_judgment(parsed: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    total_trades = metric_value(parsed, "mt5.tester_report.total_trades")
...
def current_candidate_registry_rows() -> list[dict[str, str]]:
    registry = base.read_csv_rows(REPO_ROOT / CANDIDATE_REGISTRY)
    routing_summary = base.load_yaml(REPO_ROOT / ROUTING_SUMMARY) if (REPO_ROOT / ROUTING_SUMMARY).exists() else {}
    opened_ids = set(routing_summary.get("opened_candidate_ids") or [])
    if opened_ids:
        return [
            row
            for row in registry
            if row.get("campaign_id") == CAMPAIGN_ID and row.get("candidate_id") in opened_ids
        ]
    return [
        row
        for row in registry
        if row.get("campaign_id") == CAMPAIGN_ID
        and row.get("status") == "candidate_manifest_opened_l5_runtime_evidence_pending"
    ]
...
def build_evidence_rows(*, write_parse_summaries: bool) -> tuple[list[dict[str, Any]], dict[str, list[dict[str, Any]]]]:
    ...
    for registry_row in current_candidate_registry_rows():
...
def build_candidate_evidence_summary(candidate_id: str, rows: list[dict[str, Any]], ended_at_utc: str) -> dict[str, Any]:
    ...
    return {
        ...
        "status": result,
        "claim_boundary": NEGATIVE_CLAIM_BOUNDARY if result.startswith("negative") else CLAIM_BOUNDARY,
        "source_of_truth_paths": [candidate_evidence_path(candidate_id).as_posix(), ROUTING_SUMMARY.as_posix(), ROUTING_INDEX.as_posix()],
        "writer_owned_outputs": [candidate_evidence_path(candidate_id).as_posix()],
        ...
        "writer_scope_self_check": {
            "status": "pending_artifact_registry_projection",
            "writer_contract_version": WRITER_CONTRACT_VERSION,
            "validation_depth": VALIDATION_DEPTH,
        },
    }
...
def build_summary(
    ...
    candidate_results = {
        candidate_id: candidate_result(candidate_rows)[0] for candidate_id, candidate_rows in by_candidate.items()
    }
    ...
    positive_observation_ids = [
        candidate_id for candidate_id, result in candidate_results.items() if result == "positive_runtime_observation_not_economics_pass"
    ]
    result_status = (
        "wave02_execution_liquidity_l5_candidate_runtime_evidence_observed_no_l5_candidates_boundary_decision_pending"
        if not positive_observation_ids
        else STATUS
    )
    claim_boundary = NEGATIVE_CLAIM_BOUNDARY if not positive_observation_ids else CLAIM_BOUNDARY
    return {
        "version": "wave02_execution_liquidity_l5_candidate_runtime_evidence_summary_v1",
        "summary_id": "wave02_execution_liquidity_l5_candidate_runtime_evidence_summary_v0",
        ...
        "id_chain": {
            ...
            "sweep_id": SWEEP_ID,
            "run_id": None,
            "artifact_id": "artifact_wave02_execution_liquidity_l5_candidate_runtime_evidence_summary_v0",
            "bundle_id": None,
            "candidate_id": None,
        },
        "campaign_id": CAMPAIGN_ID,
        ...
        "source_of_truth_paths": [ROUTING_SUMMARY.as_posix(), ROUTING_INDEX.as_posix(), CANDIDATE_REGISTRY.as_posix()],
        "writer_owned_outputs": [EVIDENCE_SUMMARY.as_posix(), EVIDENCE_INDEX.as_posix(), EVIDENCE_CLOSEOUT.as_posix()],
        "validation_depth": VALIDATION_DEPTH,
        "non_pytest_smokes": list(NON_PYTEST_SMOKES),
        "skipped_broad_validations": list(SKIPPED_BROAD_VALIDATIONS),
        "broad_validation_escalation_reason": BROAD_VALIDATION_ESCALATION_REASON,
        "counts": {
            ...
            "positive_runtime_observation_count": len(positive_observation_ids),
        ...
        "positive_runtime_observation_ids": positive_observation_ids,
        "l5_candidate_ids": [],
        "judgment": {
            "judgment_label": (
                "negative_runtime_evidence_no_l5_candidates"
                if not positive_observation_ids
                else "positive_runtime_observation_no_economics_pass"
            ),
            "candidate_count": len(by_candidate),
            "l5_candidate_count": 0,
            ...
            "claim_boundary": claim_boundary,
            "missing_evidence": [
                "no_selected_baseline_or_runtime_authority_from_candidate_specific_report_parse",
        ...
        "provenance": {
            ...
            "source_of_truth_paths": [EVIDENCE_SUMMARY.as_posix(), EVIDENCE_INDEX.as_posix(), EVIDENCE_CLOSEOUT.as_posix()],
            "environment_summary": {
            ...
            "lineage_judgment": "candidate_l5_runtime_evidence_written_from_candidate_manifest_attempt_ids_and_tester_report_kpi_parse",
            "claim_boundary": claim_boundary,
        },
        "artifact_outputs": {
        ...
        "prevention_memory": [
            ...
            "Candidate runtime evidence is not a selected baseline, runtime authority, live readiness, or Goal Achieve.",
            "Operational validation remains unopened.",
        ],
        "unresolved_blockers": ["Wave02_execution_liquidity_campaign_boundary_decision_pending"],
        "reopen_conditions": [
            "rerun candidate runtime evidence if candidate registry, tester report KPI parser, or candidate attempt ids change",
...
def update_candidate_summaries(summary: dict[str, Any], by_candidate: dict[str, list[dict[str, Any]]]) -> None:
    ...
    for candidate_id, rows in by_candidate.items():
        ...
        candidate["claim_boundary"] = NEGATIVE_CLAIM_BOUNDARY if result.startswith("negative") else CLAIM_BOUNDARY
        ...
        candidate["writer_scope_self_check"] = {
            "status": "pending_l5_evidence_artifact_registry_projection",
            "writer_contract_version": WRITER_CONTRACT_VERSION,
            "validation_depth": VALIDATION_DEPTH,
        ...
        safe_write_yaml(
            REPO_ROOT / candidate_evidence_path(candidate_id),
            build_candidate_evidence_summary(candidate_id, rows, summary["ended_at_utc"]),
        )
...
def upsert_candidate_registry(summary: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        if candidate_id in summary["candidate_results"]:
            result = summary["candidate_results"][candidate_id]
            row["status"] = result
            row["claim_boundary"] = NEGATIVE_CLAIM_BOUNDARY if result.startswith("negative") else CLAIM_BOUNDARY
            ...
            row["risk_notes"] = "candidate_runtime_evidence_no_l5_candidate_no_economics_pass"
...
def upsert_artifact_registry(summary: dict[str, Any]) -> None:
    ...
    def put(artifact_id: str, artifact_type: str, path: Path, notes: str, claim_boundary: str | None = None) -> None:
        full = REPO_ROOT / path
        by_id[artifact_id] = {
            **{key: "" for key in fieldnames},
            ...
            "consumer": NEXT_WORK_ITEM_ID,
            "claim_boundary": claim_boundary or summary["claim_boundary"],
            "notes": notes,
        }

    put(
        "artifact_wave02_execution_liquidity_l5_candidate_runtime_evidence_summary_v0",
        "l5_candidate_runtime_evidence_summary",
        EVIDENCE_SUMMARY,
        "Wave02 ELQ candidate-specific tester report KPI runtime evidence summary",
    )
    put(
        "artifact_wave02_execution_liquidity_l5_candidate_runtime_evidence_index_v0",
        "l5_candidate_runtime_evidence_index",
        EVIDENCE_INDEX,
        "Wave02 ELQ candidate-specific tester report KPI runtime evidence index",
    )
    put(
        "artifact_wave02_execution_liquidity_l5_candidate_runtime_evidence_closeout_v0",
        "work_closeout",
        EVIDENCE_CLOSEOUT,
        "Wave02 ELQ candidate runtime evidence closeout",
    )
    for path_value in summary["artifact_outputs"]["candidate_evidence_summaries"]:
        path = Path(path_value)
        candidate_id = path.parent.name
        result = summary["candidate_results"].get(candidate_id, "")
        claim_boundary = NEGATIVE_CLAIM_BOUNDARY if result.startswith("negative") else CLAIM_BOUNDARY
        put(
            f"artifact_{candidate_id}_l5_runtime_evidence_summary_v0",
            "candidate_l5_runtime_evidence_summary",
            path,
            "candidate-level ELQ L5 runtime evidence summary",
            claim_boundary,
        )
    registry_rows = base.read_csv_rows(REPO_ROOT / CANDIDATE_REGISTRY)
    registry_by_id = {row["candidate_id"]: row for row in registry_rows if row.get("candidate_id")}
    for candidate_id, result in summary["candidate_results"].items():
        registry_row = registry_by_id.get(candidate_id) or {}
        summary_path = registry_row.get("summary_path")
        if not summary_path:
            continue
        claim_boundary = NEGATIVE_CLAIM_BOUNDARY if str(result).startswith("negative") else CLAIM_BOUNDARY
        put(
            f"artifact_{candidate_id}_summary_v0",
            "candidate_summary",
            Path(summary_path),
            "candidate summary updated by ELQ L5 runtime evidence writer",
            claim_boundary,
        )
...
def next_work_payload(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "acceptance_criteria": [
            "decide whether to close Wave02 ELQ campaign, rotate surface, or open bounded repair work",
            "keep selected baseline, runtime authority, economics pass, live readiness, and Goal Achieve forbidden",
            "commit/push only at campaign boundary if closing and user boundary policy allows it",
        ],
        "claim_boundary": NEXT_CLAIM_BOUNDARY,
        "next_action": NEXT_ACTION,
        "status": "wave02_execution_liquidity_campaign_boundary_decision_pending",
        "source_of_truth_paths": [EVIDENCE_SUMMARY.as_posix(), EVIDENCE_INDEX.as_posix(), EVIDENCE_CLOSEOUT.as_posix()],
        "writer_owned_outputs": ["campaign boundary decision or closeout record"],
        "validation_depth": VALIDATION_DEPTH,
        "non_pytest_smokes": list(NON_PYTEST_SMOKES),
        "skipped_broad_validations": list(SKIPPED_BROAD_VALIDATIONS),
        "broad_validation_escalation_reason": BROAD_VALIDATION_ESCALATION_REASON,
        ...
        "current_truth": {
            ...
            "positive_runtime_observation_ids": summary["positive_runtime_observation_ids"],
        },
        ...
        "summary": "Wave02 ELQ campaign boundary decision pending after candidate runtime evidence; no selected baseline/runtime authority/economics pass.",
    }
...
def update_control_records(summary: dict[str, Any]) -> None:
    ...
    goal["claim_boundary"] = next_work["claim_boundary"]
    goal["next_work_item"] = {
        "work_item_id": next_work["work_item_id"],
        "path": NEXT_WORK_ITEM.as_posix(),
        "summary": "Wave02 ELQ campaign boundary decision pending after candidate runtime evidence.",
    }
    ...
    counts["wave02_execution_liquidity_l5_candidate_runtime_evidence"] = summary["counts"]
    ...
    elq["l5_candidate_runtime_evidence_summary"] = EVIDENCE_SUMMARY.as_posix()
    elq["l5_candidate_runtime_evidence_index"] = EVIDENCE_INDEX.as_posix()
    elq["l5_candidate_runtime_evidence_status"] = summary["status"]
    elq["l5_candidate_runtime_evidence_counts"] = summary["counts"]
        ...
        for row in goal_rows:
            if row.get("goal_id") == GOAL_ID:
                ...
                if "claim_boundary" in row:
                    row["claim_boundary"] = next_work["claim_boundary"]
        if goal_rows:
...
def finalize_candidate_self_checks(summary: dict[str, Any]) -> None:
    registry_rows = base.read_csv_rows(REPO_ROOT / CANDIDATE_REGISTRY)
    registry_by_id = {row["candidate_id"]: row for row in registry_rows if row.get("candidate_id")}
    artifact_rows = base.read_csv_rows(REPO_ROOT / ARTIFACT_REGISTRY)
    artifacts = {row.get("artifact_id"): row for row in artifact_rows}
    checked_at = utc_now()
    for candidate_id in summary["candidate_results"]:
        candidate_path = Path(registry_by_id[candidate_id]["summary_path"])
        payload = base.load_yaml(REPO_ROOT / candidate_path)
        evidence_path = candidate_evidence_path(candidate_id)
        artifact_id = f"artifact_{candidate_id}_l5_runtime_evidence_summary_v0"
        failures: list[str] = []
        artifact = artifacts.get(artifact_id)
        if not artifact:
            failures.append(f"missing_artifact_registry:{artifact_id}")
        elif artifact.get("sha256") != base.sha256(REPO_ROOT / evidence_path):
            failures.append(f"registry_hash_mismatch:{artifact_id}")
        if payload.get("runtime_evidence_result", {}).get("economics_pass") is not False:
            failures.append("economics_pass_not_false")
        if payload.get("runtime_evidence_result", {}).get("runtime_authority") is not False:
        ...
        payload["writer_scope_self_check"] = {
            ...
            "skipped_broad_validations": list(SKIPPED_BROAD_VALIDATIONS),
            "broad_validation_escalation_reason": BROAD_VALIDATION_ESCALATION_REASON,
            "source_of_truth_paths": [candidate_path.as_posix(), evidence_path.as_posix()],
            "writer_owned_outputs": [candidate_path.as_posix(), evidence_path.as_posix()],
        ...
        safe_write_yaml(REPO_ROOT / candidate_path, payload)
...
def writer_scope_self_check(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    rows = base.read_csv_rows(REPO_ROOT / EVIDENCE_INDEX) if (REPO_ROOT / EVIDENCE_INDEX).exists() else []
    if len(rows) != summary["counts"]["period_evidence_row_count"]:
        failures.append("evidence_index_row_count_mismatch")
    if summary["counts"]["l5_candidate_count"] != 0:
        failures.append("l5_candidate_count_nonzero_without_campaign_boundary_decision")
    for path_value in summary["artifact_outputs"]["candidate_evidence_summaries"]:
    ...
    for row in registry_rows:
        candidate_id = row.get("candidate_id")
        if candidate_id in summary["candidate_results"] and row.get("status") != summary["candidate_results"][candidate_id]:
            failures.append(f"candidate_registry_status_mismatch:{candidate_id}")
    artifact_rows = base.read_csv_rows(REPO_ROOT / ARTIFACT_REGISTRY)
    artifacts = {row.get("artifact_id"): row for row in artifact_rows}
    for artifact_id, path in [
        ("artifact_wave02_execution_liquidity_l5_candidate_runtime_evidence_summary_v0", EVIDENCE_SUMMARY),
        ("artifact_wave02_execution_liquidity_l5_candidate_runtime_evidence_index_v0", EVIDENCE_INDEX),
        ("artifact_wave02_execution_liquidity_l5_candidate_runtime_evidence_closeout_v0", EVIDENCE_CLOSEOUT),
    ]:
...
def build_command_argv(args: argparse.Namespace) -> list[str]:
    command = ["python", "foundation/pipelines/prepare_wave02_execution_liquidity_l5_candidate_runtime_evidence.py"]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare Wave02 ELQ candidate-specific L5 runtime evidence from tester reports.")
...
def main(argv: list[str] | None = None) -> int:
    routing_writer.pair_writer.runtime_writer.configure_base()
    ...
    safe_write_yaml(REPO_ROOT / EVIDENCE_CLOSEOUT, build_closeout(summary))
    upsert_artifact_registry(summary)
    finalize_candidate_self_checks(summary)
    upsert_artifact_registry(summary)
    if args.write_control_records:
        update_control_records(summary)
    self_check = writer_scope_self_check(summary)

...
WORK_ITEM_ID = "work_wave03_bounded_synthesis_special_mixing_mix2_l4_runtime_execution_v0"
SUBWORK_ID = "work_wave03_bounded_synthesis_special_mixing_mix2_l4_strategy_tester_execution_v0"
...
SUMMARY_ID = "wave03_bounded_synthesis_mix2_l4_runtime_execution_summary_v0"
CLAIM_BOUNDARY = (
    "wave03_bounded_synthesis_mix2_l4_score_runtime_observation_only_no_runtime_authority_"
    "no_economics_pass_no_candidate_no_selected_baseline_no_live_readiness_no_goal_achieve"
)
SUMMARY_CLAIM_BOUNDARY = (
    "wave03_bounded_synthesis_mix2_l4_runtime_execution_progress_only_no_runtime_authority_"
    "no_economics_pass_no_candidate_no_selected_baseline_no_live_readiness_no_goal_achieve"
)
...
CLOSEOUT_PATH = Path(
    ...
    "work_wave03_bounded_synthesis_special_mixing_mix2_l4_strategy_tester_execution_v0_closeout.yaml"
)
...
BROAD_VALIDATION_ESCALATION_REASON = "none_mix2_runtime_execution_progress_no_protected_claim"
FORBIDDEN_CLAIMS = [
    "selected_baseline",
    "operating_reference",
    "operating_promotion",
...
def build_command_argv(args: Any) -> list[str]:
    ...
    if len(command) > 1:
        command[1] = "foundation/pipelines/run_wave03_bounded_synthesis_mix2_l4_mt5_attempts.py"
    return command
...
def writer_contract_fields(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "progress_effect": "bounded_synthesis_mix2_l4_runtime_probe_attempt_executed_or_recorded",
        "next_executable_action": next_action,
        "experiment_or_boundary_effect": "mix2_runtime_probe_attempt_evidence_recorded_without_protected_claim",
...
def runtime_execution_next_action(summary: dict[str, Any]) -> str:
    if all_prepared_attempts_executed(summary):
        return (
            "write mix-2 L4 pair judgment, KPI refresh, and proxy-MT5 intent behavior parity summary "
            "before mix-3 or bounded synthesis closeout"
        )
    return "continue running remaining prepared bounded synthesis mix-2 L4 Strategy Tester attempts"


def runtime_execution_blockers(summary: dict[str, Any]) -> list[str]:
    if not all_prepared_attempts_executed(summary):
        return [
            "mix2_remaining_prepared_L4_attempts",
            "mix2_runtime_kpi_and_proxy_mt5_comparison_pending_until_L4",
        ]
    blockers = [
        "mix2_L4_pair_judgment_pending",
        "mix2_proxy_MT5_intent_behavior_parity_pending",
        "mix2_KPI_triad_refresh_pending",
    ]
...
def normalize_compile_summary(repo_root: Path, compile_summary: dict[str, Any]) -> dict[str, Any]:
    payload = _ORIGINAL_NORMALIZE_COMPILE_SUMMARY(repo_root, compile_summary)
    payload.update(
        {
            "version": "wave03_bounded_synthesis_mix2_l4_runtime_execution_compile_summary_v1",
            ...
            "progress_effect": "mix2_ea_binary_preflight_recorded_for_l4_runtime_probe",
            "next_executable_action": "continue running remaining prepared bounded synthesis mix-2 L4 Strategy Tester attempts",
            "experiment_or_boundary_effect": "mix2_ea_compile_or_binary_preflight_recorded_without_runtime_or_economics_claim",
            "source_of_truth_paths": [
    ...
    payload["next_action_or_reopen_condition"] = (
        "continue running remaining prepared bounded synthesis mix-2 L4 Strategy Tester attempts"
        if not payload.get("unresolved_blockers_or_none")
        else "repair EA compile or binary availability before mix-2 terminal execution"
    )
...
def normalize_attempt_outputs(repo_root: Path, row: dict[str, str], execution_row: dict[str, Any]) -> dict[str, Any]:
    ...
    for filename, version in [
        ("terminal_run_summary.yaml", "wave03_bounded_synthesis_mix2_l4_terminal_run_summary_v1"),
        ("score_telemetry_summary.yaml", "wave03_bounded_synthesis_mix2_l4_score_telemetry_summary_v1"),
        ("score_diagnostic_summary.yaml", "wave03_bounded_synthesis_mix2_l4_score_diagnostic_summary_v1"),
    ]:
        path = root / filename
        if not path.exists():
            continue
    ...
    routing.update(
        {
            ...
            "routing_scope": "wave03_bounded_synthesis_mix2_l4_split_runtime_score_probe_execution",
    ...
    memory = "Mix-2 runtime execution keeps proxy intent behavior parity pending until MT5 telemetry rows exist."
    if memory not in prevention:
        prevention.append(memory)
    parity["comparison_class"] = "pending_mix2_pair_aggregation_and_intent_behavior_parity_after_L4_period_roles"
    parity["follow_up_action"] = "refresh KPI triad and write proxy-MT5 intent behavior parity after mix-2 L4 telemetry"
    runner.base.write_yaml(manifest_path, manifest)
...
def normalize_summary(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    summary.update(
        {
            "version": "wave03_bounded_synthesis_mix2_l4_runtime_execution_summary_v1",
    ...
    summary["proxy_runtime_parity"] = {
        ...
        "next_action": "write proxy-MT5 intent behavior parity after mix-2 L4 runtime rows are available",
    }
    summary["kpi_ledger"] = {
        "required_triad": [
        ...
        "next_action": "refresh KPI triad after mix-2 L4 runtime evidence",
    }
...
def build_closeout(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    missing = (
        ["remaining_prepared_mix2_L4_attempts"]
        if summary["status"] == runner.base.PARTIAL_STATUS
        else ["mix2_L4_pair_judgment_pending", "mix2_proxy_MT5_intent_behavior_parity_pending", "mix2_KPI_triad_refresh_pending"]
    )
    if not (summary.get("runtime_completion") or {}).get("runtime_probe_complete"):
...
def campaign_status(summary: dict[str, Any]) -> str:
    if summary["status"] == runner.base.PARTIAL_STATUS:
        return "wave03_bounded_synthesis_mix2_l4_terminal_execution_in_progress"
    return "wave03_bounded_synthesis_mix2_l4_pair_judgment_kpi_parity_required_next"
...
def update_control_records(repo_root: Path, summary: dict[str, Any]) -> None:
    ...
    missing_material = ["remaining_prepared_mix2_L4_attempts"] if summary["status"] == runner.base.PARTIAL_STATUS else blockers
    reopen_conditions = (
        ["continue terminal execution for remaining prepared mix-2 validation/research_oos attempts"]
        if summary["status"] == runner.base.PARTIAL_STATUS
        else [
            "write mix-2 l4_pair_judgment_summary and l4_pair_judgment_index",
    ...
    goal.setdefault("wave03_bounded_synthesis_mix2_l4_runtime_execution", {}).update(
        {
            "l4_runtime_execution_summary": RUNTIME_SUMMARY.as_posix(),
            "l4_runtime_execution_status": summary["status"],
            "l4_runtime_execution_counts": copy.deepcopy(summary["counts"]),
    ...
    workspace = runner.base.load_yaml(repo_root / WORKSPACE_STATE)
    workspace.update(
        {
            ...
            "next_action": next_action,
            "current_claim_boundary": summary["claim_boundary"],
            "unresolved_blockers": blockers,
        }
    )
    workspace.setdefault("summary_counts", {})["wave03_bounded_synthesis_mix2_l4_runtime_execution"] = {
    ...
    if campaign_path.exists():
        ...
        campaign.setdefault("mix2_l4_runtime_execution", {}).update(
            {
                "status": status,
                "runtime_execution_summary": RUNTIME_SUMMARY.as_posix(),
                "runtime_execution_index": RUNTIME_INDEX.as_posix(),
    ...
    registry_updates = {
        ...
        "notes": "Bounded synthesis mix-2 L4 runtime execution progress recorded; KPI and intent parity remain closeout gates.",
    }

...
- Keep project-wide synchronization proportional. Scoped campaign-local closeout PRs default to relevant partial pytest and validators only when changed paths stay within campaign-local evidence, memory clues/negative memory, and approved campaign/clue/negative/artifact registers, and no protected claim is introduced. Full pytest, full active-record validation, global registry regeneration, and broad hash sync are for workflow, dependency, policy, control-plane, validator, evaluator, MT5 helper, goal/wave/candidate register, workspace, goal, wave, MT5 attempt receipt, source-of-truth drift, user-directed full regression, or protected runtime/economics/handoff/baseline/production/reviewed-or-verified claims; campaign closeout escalates when one of those conditions applies.

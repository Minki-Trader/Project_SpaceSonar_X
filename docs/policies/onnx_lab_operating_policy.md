...
- Keep project-wide synchronization proportional. Scoped campaign-local closeout PRs default to relevant partial pytest and validators only when changed paths stay campaign-local and no protected runtime/economics/live-readiness claim is introduced. Full pytest, full active-record validation, global registry regeneration, and broad hash sync are for workflow, policy, control-plane, validator, evaluator, register, workspace, goal, wave, MT5 attempt receipt, source-of-truth drift, user-directed full regression, or protected runtime/economics/handoff claims; campaign closeout escalates when one of those conditions applies.

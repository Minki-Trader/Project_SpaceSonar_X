...
For campaign-closeout PRs, the default required merge checks are the scoped control-plane checks recorded in `docs/policies/github_branch_protection_required.yaml`. The complete pytest regression is not a default campaign-local closeout merge gate. The `ci-scope-gate` classifier must require a successful manual `full-regression` run, or a user-approved `docs/ci/full_regression_override.yaml`, for protected paths, wave or goal closeout, workflow/policy/control-plane/validator changes, global register/workspace changes, MT5 attempt receipts, dependency changes, or protected runtime/economics/live-readiness/selected-baseline/production/reviewed-or-verified-pass claims.

...
from foundation.pipelines.prepare_wave01_session_transition_l4_mt5_attempts import (
    CLAIM_BOUNDARY,
...
def test_session_transition_attempt_preparer_configures_session_paths() -> None:
    original = {
    ...
    try:
        ...
        assert "session_transition" in CLAIM_BOUNDARY
...
def test_session_transition_attempt_preparer_normalizes_summary_identity() -> None:
    summary = {
        ...
        "counts": {"prepared_attempt_count": 20},
    }

    normalized = normalize_summary(summary)

    assert normalized["summary_id"] == SUMMARY_ID
    assert normalized["version"] == "wave01_session_transition_l4_mt5_attempt_preparation_summary_v1"
    assert normalized["claim_boundary"] == CLAIM_BOUNDARY
    assert normalized["attempt_depth"] == "bounded_attempt_manifest_preparation"
